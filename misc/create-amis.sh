...
# taken from https://github.com/NixOS/nixpkgs/blob/master/nixos/maintainers/scripts/ec2/create-amis.sh
...
# configuration
var ${state_dir:=./ec2-images}
var ${home_region:=eu-west-1}
var ${bucket:=nixos-amis}
var ${service_role_name:=vmimport}
var ${image_s3_key:=DEFINE_IMAGE_S3_KEY}
var ${project_name:=DEFINE_PROJECT_NAME}
...
image_name="NixOS-${project_name}-${image_label}-${image_system}"
...
upload_image() {
    ...
    for image_file in "${image_files[@]}"; do
        ...
        local state_key="$region.$image_system"
        local task_id
        ...
        if [ -z "$task_id" ]; then
            ...
            if ! aws s3 ls --region "$region" "s3://${bucket}/${image_s3_key}" >&2; then
                log "Image missing from aws, should have been uploaded from terraform"
                exit 1
            fi

            log "Importing image from S3 path s3://$bucket/$image_s3_key"
