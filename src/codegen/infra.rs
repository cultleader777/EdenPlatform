...
use crate::{static_analysis::CheckedDB, database::TableRowPointerServer};

use super::{Directory, SshKeysSecrets, NixCacheKeys, l1_provisioning::hardware::zfs_args_qemu};

pub fn configure_server_ip_interfaces(db: &CheckedDB, server: TableRowPointerServer, res: &mut String) {
    let networks_to_explicitly_assign_ip = [
        "dcrouter",
        "lan",
    ];
    // we perform check beforehand if DHCP given ip is what we expect
    // then disable DHCP on all given interfaces during provisioning
    write!(res, r#"
...
"#).unwrap();
    ...
    for ni in db.db.server().c_children_network_interface(server) {
        ...
        if networks_to_explicitly_assign_ip.contains(&net_name.as_str()) || (explicit_internet_ip && is_internet) {
            let iface_name = db.db.network_interface().c_if_name(*ni);
            if iface_name.contains(":") {
                // subinterface
                continue;
            }
            let iface_ip = db.db.network_interface().c_if_ip(*ni);
            let prefix_length = db.db.network_interface().c_if_prefix(*ni);
            write!(res, r#"
    networking.interfaces.{iface_name}.ipv4.addresses = [
      {{ address = "{iface_ip}"; prefixLength = {prefix_length}; }}
"#).unwrap();
            for ni_child in db.db.server().c_children_network_interface(server) {
                if ni == ni_child {
                    continue;
                }

                let child_if_name = db.db.network_interface().c_if_name(*ni_child);
                let starts_with = child_if_name.starts_with(iface_name);
                if starts_with && child_if_name.len() > iface_name.len()
                    && child_if_name.as_bytes()[iface_name.len()] == b':'
                {
                    // subinterface
                    let child_iface_ip = db.db.network_interface().c_if_ip(*ni_child);
                    let child_prefix_length = db.db.network_interface().c_if_prefix(*ni_child);
            write!(res, r#"
      {{ address = "{child_iface_ip}"; prefixLength = {child_prefix_length}; }}
"#).unwrap();
                }
...
pub(crate) fn base_machine_template(
    db: &CheckedDB,
    gw_ip: &str,
    nixpkgs_hash: &str,
    nixpkgs_tarball_hash: &str,
    nixpkgs_version: &str,
    root_ssh_keys: &SshKeysSecrets,
    nix_cache_keys: &NixCacheKeys,
) -> String {
    let zfs_args = zfs_args_qemu();
    let mut res = format!(
        r#"
let
  pkgs = import (fetchTarball{{ url = "https://github.com/NixOS/nixpkgs/archive/{nixpkgs_hash}.tar.gz"; sha256 = "{nixpkgs_tarball_hash}"; }}) {{}};
  lib = pkgs.lib;
in
...
"#;
    ...
    res += "      \"";
    res += &nix_cache_keys.public_nix_cache_key.value();
    res += "\"\n";
    res += r#"
    ];
  };
...
"#;
    ...
    for key in db.db.admin_ssh_keys().rows_iter() {
        if keys.insert(db.db.admin_ssh_keys().c_contents(key).clone()) {
            res += "    \"";
            res += db.db.admin_ssh_keys().c_contents(key);
            res += "\"\n";
        }
    }
    ...
    if keys.insert(root_ssh_keys.public_root_ssh_key.value().clone()) {
        res += "      \"";
        res += root_ssh_keys.public_root_ssh_key.value().trim();
        res += "\"\n";
    }
...
/// Returns virbr names created
pub(crate) fn generate_vm_network_xml(
    db: &CheckedDB,
    networks_dir: &mut Directory,
) {
    for n in db.sync_res.network.libvirt_network_topology.networks.values() {
        let virbr_name = &n.libvirt_name;
        let gw_ip = &n.gw_ip;
        let netmask = n.ip_net.netmask();
        let mut res = format!(
            r#"
<network>
  <name>{virbr_name}</name>
  <bridge stp='off' delay='0' name='{virbr_name}'/>
"#,
        );
        ...
        if n.dhcp_enable {
            ...
            r#"
  <forward mode='nat'/>
  <ip address='{gw_ip}' netmask='{netmask}'>
    <dhcp>
"#,
            ).unwrap();

            for (server, values) in &n.servers {
                let mac_address = values.mac.to_hex_string();
                res += "      <host mac='";
                res += &mac_address;
                res += "' name='";
                res += db.db.server().c_hostname(*server);
                res += "' ip='";
                write!(&mut res, "{}", values.ip).unwrap();
                res += "' />";
                res += "\n";
            }
...
"#;
        ...
        let fname = format!("{virbr_name}.xml");
        networks_dir.create_file(&fname, res);
    }
}

pub(crate) fn ssh_config_for_colmena() -> &'static str {
    r#"IdentityFile=aux/root_ssh_key
StrictHostKeyChecking=false
UserKnownHostsFile=/dev/null
"#
}

pub(crate) fn utils_library_script() -> &'static str {
    r#"
