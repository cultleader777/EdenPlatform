...
#[test]
fn test_application_stream_wiring_enable_message_id_mismatch() {
    let err = common::assert_platform_validation_error_wcustom_data_wargs(
        common::TestArgs { add_default_global_flags: false, add_default_data: true },
        r#"
DATA STRUCT global_settings {
...
"#,
    ...
    assert_eq!(
        err,
        PlatformValidationError::ApplicationStreamWiringMessageIdEnabledMismatch {
            application_deployment: "test-depl".to_string(),
            application_name: "hello-world".to_string(),
            bad_line: "test_stream: default=>deployed_stream".to_string(),
            explanation:
                "Application expected NATS stream enable_message_id value mismatches wired NATS cluster stream enable_message_id value",
            application_expected_enable_message_id: true,
            target_deployment_stream_enable_message_id: false,
        }
    );
}
