...
# create vault kv policy for job
VAULT_POLICY=$( cat <<EOF
path "epl/data/loki/main-us-west" {
  capabilities = ["read"]
}
EOF
)
ensure_vault_policy_for_kv_exists "epl-loki-main-us-west" "$VAULT_POLICY"
# create consul kv policy epl-loki-main-us-west-loki
CONSUL_POLICY=$( cat <<EOF
key_prefix "nomad-loki/epl-loki-main-us-west-loki" {
    policy = "write"
}
EOF
)
ensure_consul_token_with_policy_exists $( echo "$MERGED_SECRET" | jq -r '.consul_token' ) 'epl-loki-main-us-west-loki' "$CONSUL_POLICY"
# pg/testdb-us-west
SEC_KEY=epl/pg/testdb-us-west
CURRENT_SECRET=$( vault kv get -format=json $SEC_KEY | jq -S '.data.data' )
CURRENT_SECRET=${CURRENT_SECRET:-\{\}}
SEC_1=$(generate_consul_acl_token)
SEC_2=$(random_password_42)
...
MERGED_SECRET=$( echo "{\"consul_token\":\"$SEC_1\",\"pg_admin_password\":\"$SEC_2\",\"pg_db_grafana_password\":\"$SEC_3\",\"pg_exporter_password\":\"$SEC_4\",\"pg_replicator_password\":\"$SEC_5\",\"pg_rewind_password\":\"$SEC_6\",\"pg_superuser_password\":\"$SEC_7\"} $CURRENT_SECRET" | jq -S -s add )
if [ "$MERGED_SECRET" != "$CURRENT_SECRET" ];
then
  echo "Secret $SEC_KEY changed"
  echo "$MERGED_SECRET" | vault kv put $SEC_KEY -
...
# create vault kv policy for job
VAULT_POLICY=$( cat <<EOF
path "epl/data/pg/testdb-us-west" {
  capabilities = ["read"]
}
EOF
)
ensure_vault_policy_for_kv_exists "epl-pg-testdb-us-west" "$VAULT_POLICY"
# create consul kv policy epl-pg-testdb-us-west
CONSUL_POLICY=$( cat <<EOF
key_prefix "epl-patroni/epl-pg-testdb-us-west" {
    policy = "write"
}

service_prefix "epl-pg-testdb-us-west" {
    policy = "write"
}
...
ensure_consul_token_with_policy_exists $( echo "$MERGED_SECRET" | jq -r '.consul_token' ) 'epl-pg-testdb-us-west' "$CONSUL_POLICY"
# tempo/r1-tempo
SEC_KEY=epl/tempo/r1-tempo
CURRENT_SECRET=$( vault kv get -format=json $SEC_KEY | jq -S '.data.data' )
CURRENT_SECRET=${CURRENT_SECRET:-\{\}}
SEC_1=$(aws_secret_key)
...
# create vault kv policy for job
VAULT_POLICY=$( cat <<EOF
path "epl/data/tempo/r1-tempo" {
  capabilities = ["read"]
}
EOF
)
ensure_vault_policy_for_kv_exists "epl-tempo-r1-tempo" "$VAULT_POLICY"
# grafana/main-us-west
SEC_KEY=epl/grafana/main-us-west
CURRENT_SECRET=$( vault kv get -format=json $SEC_KEY | jq -S '.data.data' )
CURRENT_SECRET=${CURRENT_SECRET:-\{\}}
SEC_1=$(random_password_42)
SEC_2=$( vault kv get -format=json epl/pg/testdb-us-west | jq -Srj '.data.data.pg_db_grafana_password' | sed 's/$/\\n/g' | tr -d '\n' | sed 's/..$//' )
...
# create vault kv policy for job
VAULT_POLICY=$( cat <<EOF
path "epl/data/grafana/main-us-west" {
  capabilities = ["read"]
}
EOF
)
ensure_vault_policy_for_kv_exists "epl-grafana-main-us-west" "$VAULT_POLICY"
# minio/main-us-west
SEC_KEY=epl/minio/main-us-west
CURRENT_SECRET=$( vault kv get -format=json $SEC_KEY | jq -S '.data.data' )
CURRENT_SECRET=${CURRENT_SECRET:-\{\}}
SEC_1=$(aws_secret_key)
SEC_2=$( vault kv get -format=json epl/docker-registry | jq -Srj '.data.data.minio_bucket_password' | sed 's/$/\\n/g' | tr -d '\n' | sed 's/..$//' )
SEC_3=$( vault kv get -format=json epl/loki/main-us-west | jq -Srj '.data.data.minio_bucket_password' | sed 's/$/\\n/g' | tr -d '\n' | sed 's/..$//' )
SEC_4=$( vault kv get -format=json epl/tempo/r1-tempo | jq -Srj '.data.data.minio_bucket_password' | sed 's/$/\\n/g' | tr -d '\n' | sed 's/..$//' )
MERGED_SECRET=$( echo "{\"admin_password\":\"$SEC_1\",\"minio_user_docker_registry_password\":\"$SEC_2\",\"minio_user_loki_main_us_west_password\":\"$SEC_3\",\"minio_user_tempo_r1_tempo_password\":\"$SEC_4\"} $CURRENT_SECRET" | jq -S -s add )
if [ "$MERGED_SECRET" != "$CURRENT_SECRET" ];
then
  echo "Secret $SEC_KEY changed"
  echo "$MERGED_SECRET" | vault kv put $SEC_KEY -
...
# create vault kv policy for job
VAULT_POLICY=$( cat <<EOF
path "epl/data/minio/main-us-west" {
  capabilities = ["read"]
}
EOF
)
ensure_vault_policy_for_kv_exists "epl-minio-main-us-west" "$VAULT_POLICY"
