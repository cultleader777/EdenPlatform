...
while ! curl -f http://epl-grafana-main-us-west.service.consul:3000/api/health
do
  echo Waiting for grafana healthcheck to be up...
  sleep 5
done
ADMIN_PASSWORD=$( vault kv get -field=admin_password epl/grafana/main-us-west )
...
while ! curl -f -u admin:$ADMIN_PASSWORD -XPOST -H 'Content-Type: application/json' --data @grafana-dashboard/node-exporter-full.json.fixed http://epl-grafana-main-us-west.service.consul:3000/api/dashboards/db
do
