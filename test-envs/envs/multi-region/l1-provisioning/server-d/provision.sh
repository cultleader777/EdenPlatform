...
client {
  ...
  meta = {
    ...
    "lock_epl-minio-server-d-main-us-west" = "1"
    "lock_epl-mon-server-d-am-main-us-west" = "1"
  }
...
service_registration "consul" {
  ...
  token = "543ff682-6224-46b9-b4a2-4ba4d769d47f"
}
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
oyMnrRW8fNW+vBN7xSeBzNXYEqAmODrMCyGntA+DOy4=
LilBoiPeepLikesBenzTruck
...
# NIX REGION secret_value_K17-10-in-addr-arpa--015-20142-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: EvNsMd43k3O5i5DbbnoK5R/oIvs7DC6cXL4Jiv9uqSw=
Created: 20240306032226
Publish: 20240306032226
Activate: 20240306032226
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-20142-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-20142-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-20142-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-20142-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-59549-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: bRRDlxz1VhJJDgmh7sWLrVg84qag1aM6NmVbWCCkJRU=
Created: 20240306032226
Publish: 20240306032226
Activate: 20240306032226
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-59549-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-59549-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-59549-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-59549-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-20142-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 20142, for 17.10.in-addr.arpa.
...
17.10.in-addr.arpa. IN DNSKEY 256 3 15 Sn+8jbtByI0C15ac3Y4zxoKErhonWyDAf5KKynf3Ges=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-20142-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-20142-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-20142-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-20142-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-59549-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 59549, for 17.10.in-addr.arpa.
; Created: 20240306032226 (Wed Mar  6 05:22:26 2024)
; Publish: 20240306032226 (Wed Mar  6 05:22:26 2024)
; Activate: 20240306032226 (Wed Mar  6 05:22:26 2024)
17.10.in-addr.arpa. IN DNSKEY 257 3 15 hnuAeTi9gNoGSSOXCsnZyTozZse/+h8Z7tUNHGUFNIg=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-59549-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-59549-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-59549-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-59549-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM
...
# NIX REGION secret_value_Kus-west-epl-infra-net--015-48924-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: pU1bpgTVZATflOmxAOXjYqc5YzDyRi1jZ1ytpDXhjr4=
Created: 20240306032226
Publish: 20240306032226
Activate: 20240306032226
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-48924-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-48924-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-48924-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-48924-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23495-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: HR33vk2I1uXMnvvM8jsNpgCnuFOpIdxdqvp1VJAUfrQ=
Created: 20240306032226
Publish: 20240306032226
Activate: 20240306032226
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23495-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23495-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-23495-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23495-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-48924-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 48924, for us-west.epl-infra.net.
; Created: 20240306032226 (Wed Mar  6 05:22:26 2024)
; Publish: 20240306032226 (Wed Mar  6 05:22:26 2024)
; Activate: 20240306032226 (Wed Mar  6 05:22:26 2024)
us-west.epl-infra.net. IN DNSKEY 256 3 15 vl9osF/LlZMUNx9C/JnXovaB0XtFYM2BQushh9qwfOU=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-48924-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-48924-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-48924-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-48924-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23495-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 23495, for us-west.epl-infra.net.
; Created: 20240306032226 (Wed Mar  6 05:22:26 2024)
; Publish: 20240306032226 (Wed Mar  6 05:22:26 2024)
; Activate: 20240306032226 (Wed Mar  6 05:22:26 2024)
us-west.epl-infra.net. IN DNSKEY 257 3 15 ftP7Fcgh5mOf4RFi8EJxgeqabrMRh73p84Bje168Pcg=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23495-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23495-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-23495-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23495-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

18n4YteY6Cwu
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIHI4JvP9AhB9oOjBppVdx9SJEg4uu6CUapgsBm+NoPG1oAoGCCqGSM49
AwEHoUQDQgAE7zgedXloRnkoDtvMUv9Q5ZWzg2EI/SDeRjyWhQUZwkRIA4WbStBg
GzD5IYnYqkrMtQuHK3SYPtwtykXr27kRFw==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

...
gglsb2NhbGhvc3SHBH8AAAEwCgYIKoZIzj0EAwIDSAAwRQIhAKdIwSCcHhhlqama
3FwWZJv3n/oThSCUoD9VGGYcJDqeAiBpX1BAQMQM4U0I3E1AHA6KhYAAw5EDxPnl
K+tnIqgQGA==
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIG0SbKg0c3HVMQBtQVnNc4iWFDuEP4KvmbeV6RrSu26aoAoGCCqGSM49
AwEHoUQDQgAEDz0T54QY78T89D8TY12wbyIatsrFVAP7zunkrC5eEzntISQ5Iq1X
q/HWPSzUkb9G/UwH+h+s8rCn4aaOeFTFNQ==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
oP7gjYqGwyCp47/JzKtf0syuFSb/l4FRi+ywmU1Nk00=
LilBoiPeepLikesBenzTruck
...
# ns2.epl-infra.net.	IN	A	10.18.0.13
...
# us-west.epl-infra.net.	IN	DS	23495 15 2 9D4B93959B57E99179EAE2B4C0F4241DF46B28BBBF5DE4D70749A00D03B885D6
#
...
# us-east.epl-infra.net.	IN	DS	29833 15 2 6C4BDB6FB7C0B41AF756A07FAD1D90EEBF0E920AB1D48A820E8D289EF9BE458B
#
maybe_update_dns_file  /run/named/private-epl-infra.net.zone
# $TTL 3600
...
# 13.0.18.10.in-addr.arpa.	IN	PTR	ns2.epl-infra.net.
...
maybe_update_dns_file  /run/named/private-10.in-addr.arpa.zone
# $TTL 3600
...
# server-g.us-west.epl-infra.net.	IN	A	10.18.0.12
# server-h.us-west.epl-infra.net.	IN	A	10.18.0.13
maybe_update_dns_file  /run/named/private-us-west.epl-infra.net.zone
# $TTL 3600
...
# ns2.epl-infra.net.	IN	A	77.77.77.13
# adm-alertmanager-main-us-east	IN	A	77.77.77.10
# adm-alertmanager-main-us-east	IN	A	77.77.77.11
# adm-alertmanager-main-us-west	IN	A	77.77.77.10
# adm-alertmanager-main-us-west	IN	A	77.77.77.11
# adm-consul-us-east	IN	A	77.77.77.10
# adm-consul-us-east	IN	A	77.77.77.11
# adm-consul-us-west	IN	A	77.77.77.10
# adm-consul-us-west	IN	A	77.77.77.11
# adm-grafana-main-us-west	IN	A	77.77.77.10
# adm-grafana-main-us-west	IN	A	77.77.77.11
# adm-minio-main-us-east	IN	A	77.77.77.10
# adm-minio-main-us-east	IN	A	77.77.77.11
# adm-minio-main-us-west	IN	A	77.77.77.10
# adm-minio-main-us-west	IN	A	77.77.77.11
# adm-nomad-us-east	IN	A	77.77.77.10
# adm-nomad-us-east	IN	A	77.77.77.11
# adm-nomad-us-west	IN	A	77.77.77.10
# adm-nomad-us-west	IN	A	77.77.77.11
# adm-prometheus-main-us-east	IN	A	77.77.77.10
# adm-prometheus-main-us-east	IN	A	77.77.77.11
# adm-prometheus-main-us-west	IN	A	77.77.77.10
# adm-prometheus-main-us-west	IN	A	77.77.77.11
# adm-vault-us-east	IN	A	77.77.77.10
# adm-vault-us-east	IN	A	77.77.77.11
# adm-vault-us-west	IN	A	77.77.77.10
# adm-vault-us-west	IN	A	77.77.77.11
# admin	IN	A	77.77.77.10
# admin	IN	A	77.77.77.11
maybe_update_dns_file  /run/named/public-epl-infra.net.zone
# $TTL 3600
...
# 13.77.77.77.in-addr.arpa.	IN	PTR	ns2.epl-infra.net.
maybe_update_dns_file  /run/named/public-in-addr.arpa.zone
...
zpool import -af
provision_zfs_dataset rpool acme /var/lib/acme 128k on no Yn9Ft1loG7nsTdqfecFwzEqyTDtaGsEnIOXnUpNTLc
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool mon-am /srv/volumes/mon-am 4k on yes lMNPBrm2fqZfxXzkJ78FgRozhlQGQuULShuJAXyOYl
provision_zfs_dataset rpool mon-secondary /srv/volumes/mon-secondary 4k on yes mc6Bv1vXan7IDQ0IjZW3swvLEqwhdqnXuTXHI14lDO
provision_zfs_dataset rpool nats1 /srv/volumes/nats1 4k on yes N22n7XhoBZIdx1Q1ZUAyjj39gpMMAjky4G5i7iFONS
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no N8xe9ABS1Xqzdatp9mIdNc1sIC4SMUaky9cq4GeckM
provision_zfs_dataset rpool vault /var/lib/vault 4k on no 1kupcIiteEOuPXEJdmSzMwFi6a4jRkPsXxvNHPC2qQ
...
# NIX REGION epl_nft_rules_epl-internet-fw START
       ...
       chain EPL_INTERNET_FIREWALL {
           ...
           iifname eth1 ip saddr != { 10.0.0.0/8, 172.21.0.0/16 } ip daddr != { 77.77.77.11/32 } drop comment "Disallow traffic from internet to internal networks";
       }
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-d.dc1.us-west.multi-region:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-d.dc1.us-west.multi-region: \w\a\]\u@server-d.dc1.us-west.multi-region:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-d.dc1.us-west.multi-region:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
wg set wg0 peer "stKhSSRSHNOZ3usadnQtQswYVYNmH6hf45tIs+IdMi8=" allowed-ips "172.21.7.13/32,10.18.0.0/16" endpoint "77.77.77.13:51820"
  ...
  services.frr.ospf = {
      ...
      config = ''
        ...
        interface eth0
          ...
          ip ospf message-digest-key 12 md5 TUxj4xw2HLeXLNVH
  ...
  services.frr.bgp = {
      ...
      config = ''
        ...
        router bgp 64529
          ...
          neighbor 10.17.0.12 password jjWtyk6P5I2ecJGo0fno9r1qddHTgyNLAsy9E7LJfn
          neighbor 10.17.0.12 bfd
          neighbor 172.21.7.13 remote-as 64530
          neighbor 172.21.7.13 password A36W4XhjkRsz7CcjV7bRGhOj3SX9SxlSRv2tEO8YGT
...
# NIX REGION keepalived END
    ...
    networking.interfaces.eth1.ipv4.addresses = [
      { address = "77.77.77.11"; prefixLength = 24; }
...
cp -pu /run/keys/K17-10-in-addr-arpa--015-20142-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+20142.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-59549-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+59549.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-20142-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+20142.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-59549-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+59549.key
...
cp -pu /run/keys/Kus-west-epl-infra-net--015-48924-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+48924.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-23495-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+23495.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-48924-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+48924.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-23495-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+23495.key
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"a2c1704f564d32832fb45e725f33d26f4fa3823718b5de1105c42e1bd8cb3511\",hostname=\"server-d\"} $BOOT_TIME
" > $METRICS_FILE.tmp
