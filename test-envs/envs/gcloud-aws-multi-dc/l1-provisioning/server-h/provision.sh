...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
hEyYt9t68AqYluY/sWBQ3TFOBGmsdcqSjumVScpcf3Y=
LilBoiPeepLikesBenzTruck
...
while ! ${pkgs.consul}/bin/consul kv get epl-interdc-routes/dc2/10.18.0.0p24
do
  sleep 7
done

exec ${pkgs.consul}/bin/consul watch -type=key -key=epl-interdc-routes/dc2/10.18.0.0p24 /run/current-system/sw/bin/epl-process-route-data
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-h.dc2.us-west.gcloud-aws-multi-dc:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-h.dc2.us-west.gcloud-aws-multi-dc: \w\a\]\u@server-h.dc2.us-west.gcloud-aws-multi-dc:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-h.dc2.us-west.gcloud-aws-multi-dc:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
# NIX REGION l3_vpn_hop_interface START
  ...
  systemd.services.vpn-gre-tunnel = {
    ...
    script = ''
        ip tunnel add vpnGre mode gre local 10.18.0.12 key 17
        ip addr add 10.18.128.12/17 dev vpnGre


        ip neighbor add 10.18.128.10 lladdr 10.18.0.10 dev vpnGre
        ip neighbor add 10.18.128.11 lladdr 10.18.0.11 dev vpnGre
...
consul kv get epl-interdc-routes/dc2 || epl_bootstrap_routes
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"efb6544e71e4232d1b80dcffc56995da3fd63f8841a35676eada011905f9b0eb\",hostname=\"server-h\"} $BOOT_TIME
" > $METRICS_FILE.tmp
