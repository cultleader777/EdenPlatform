...
host = "server-i.us-west.epl-infra.net"
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
dfEA1fpsPbMiDmrF8aQECv0uGbDpFbvk/40dQ4YkkEM=
LilBoiPeepLikesBenzTruck
...
while ! ${pkgs.consul}/bin/consul kv get epl-interdc-routes/dc3/10.19.0.0p24
do
  sleep 7
done

exec ${pkgs.consul}/bin/consul watch -type=key -key=epl-interdc-routes/dc3/10.19.0.0p24 /run/current-system/sw/bin/epl-process-route-data
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-i.dc3.us-west.gcloud-aws-multi-dc:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-i.dc3.us-west.gcloud-aws-multi-dc: \w\a\]\u@server-i.dc3.us-west.gcloud-aws-multi-dc:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-i.dc3.us-west.gcloud-aws-multi-dc:\u:\w\007\]$PS1"
       fi
     fi
   '';
     ...
     systemd.services.l1-fast-agent = {
       ...
       ''
         ...
         # wait for consul to become available
         while ! ${pkgs.consul}/bin/consul kv get epl-l1-plans/server-i
         do
           sleep 7
         done

         ${pkgs.consul}/bin/consul watch \
           -type=key -key=epl-l1-plans/server-i \
           ${l1Checker}/checker \
...
view lan {
    ...
    networking.interfaces.eth0.ipv4.addresses = [
      { address = "10.19.0.12"; prefixLength = 24; }
...
if [[ ! $( timeout 0.5s dig @127.0.0.1 server-i.us-west.epl-infra.net +dnssec +short ) ]]
then
...
consul kv get epl-interdc-routes/dc3 || epl_bootstrap_routes
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"3fb38dacc62a2bec506041172ee0ce7bf0aad2603e0d77e3fdb3448806e19f2d\",hostname=\"server-i\"} $BOOT_TIME
" > $METRICS_FILE.tmp
