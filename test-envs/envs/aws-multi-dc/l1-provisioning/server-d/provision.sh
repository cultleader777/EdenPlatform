...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
boc0WpBWCcpNRhTj+MGm2rTQhRAL+ccdWG0F71em9rw=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
eLRHiTYfa7G9r6CmFDq53MZVnpgHr52+kqBnEIcGHUg=
LilBoiPeepLikesBenzTruck
...
provision_zfs_dataset rpool nats1 /srv/volumes/nats1 4k on yes 0v3n2pea62kKpwAFQTDw9gS6mvrnBojLE75hjnIn8p
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no LpyMFIvyLEExTOmb6AZJGo3SyTyEfmMvfXdU0B1UUz
...
# ROUTES CREATE
ip route add 0.0.0.0/0 via 10.18.0.11
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-d.dc2.us-west.aws-multi-dc:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-d.dc2.us-west.aws-multi-dc: \w\a\]\u@server-d.dc2.us-west.aws-multi-dc:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-d.dc2.us-west.aws-multi-dc:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"2bdcfb0232f6f9d0e8b40b094ddfce9c0559dae0eecf1479eefb8c7bc9b5c69d\",hostname=\"server-d\"} $BOOT_TIME
" > $METRICS_FILE.tmp
