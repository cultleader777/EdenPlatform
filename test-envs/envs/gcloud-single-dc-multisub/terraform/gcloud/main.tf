...
resource "google_compute_subnetwork" "us-west1-b_10-17-1" {
  project       = "test1-406308"
  name          = "us-west1-b-10-17-1-subnet"
  ip_cidr_range = "10.17.1.0/24"
  network       = google_compute_network.main-vpc.self_link
  region        = "us-west1"
}
...
resource "google_compute_instance" "server-c" {
  ...
  network_interface {
    nic_type = "GVNIC"
    subnetwork = google_compute_subnetwork.us-west1-b_10-17-1.self_link
    network_ip = "10.17.1.10"
    access_config {
