...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
{
  "acl": {
    ...
    "enabled": true,
    "tokens": {
      "agent": "ed753f50-b6a7-4a9e-95e9-43abcde31f25",
      "default": "8ba69b97-918b-4960-99cd-b5f6a9611a41"
    }
  },
  "addresses": {
    "dns": "127.0.0.1",
  ...
  "encrypt": "uImJae4LU7Pc1+J3/IKp2AXO9AcMA6ggWGw+Rv95F4Y=",
...
consul {
    ...
    cert_file = ""
    key_file = ""
    token = "d6b78bd1-8b27-411e-a55b-7dd14aca05b5"
...
server {
    ...
    encrypt = "X/wjfYonq/qvDsILSfAIsc74V9Oag93NiFFPOvrnXJI="
...
service_registration "consul" {
  ...
  token = "d10fbf1f-70c8-4e85-bdff-7187c2eaec91"
}
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
T8kdhH2nzqnCAJSavoP7Ig48yYIROGaRGty6/3+QFJ0=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
MAi0y70Q7ATOEXxVZ/lwWrBPOuHD4YcPtzTFR8HJfms=
LilBoiPeepLikesBenzTruck
...
MHcCAQEEICbOpPWufMQuB53fwMIdgfOVR4oKiIKvrJJkPH8Zuh3foAoGCCqGSM49
AwEHoUQDQgAE8D5X5RNF8Ioyf9Yc33Bw9eNqlUEf3j6nRbk/im6Gd1Naji71/sBo
i5iRZ0ncIgH8FTuHA+drmRexvxsw09aZZg==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
# NIX REGION secret_value_K10-in-addr-arpa--015-21012-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: bhuX1g5o0/v1RSN8XGV+wtnyK1IUtUoDb7bDfELt3Fs=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-21012-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-21012-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-21012-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-21012-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-28404-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: H6coAdhWkH9xWiazzo2lri8NfJeopF/8vEswv+ZVKiI=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-28404-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-28404-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-28404-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-28404-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-21012-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 21012, for 10.in-addr.arpa.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
10.in-addr.arpa. IN DNSKEY 256 3 15 L2QQGmLcXIYtnI6RZoaf2EPhsYabRlzv7rwjOevuoRQ=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-21012-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-21012-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-21012-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-21012-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-28404-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 28404, for 10.in-addr.arpa.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
10.in-addr.arpa. IN DNSKEY 257 3 15 dtT6lBJJpxBrviwLFIVVHZGySeT/ApDPkgCqJn5RGNM=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-28404-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-28404-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-28404-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-28404-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-35959-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: y2XgcCGgXAtUICqF1+Tv03qpIki8NJT7dhij7fEIxIY=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-35959-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-35959-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-35959-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-35959-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-46721-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: rdDgfV7ab0fcT2ChsKWnmivU8OScrL+Ub1udLOhZkt0=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-46721-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-46721-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-46721-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-46721-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-35959-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 35959, for 17.10.in-addr.arpa.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
17.10.in-addr.arpa. IN DNSKEY 256 3 15 UvN4lct43Zh3n3cVyFuV50BGw1iDKpWlpX5NuSRXktc=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-35959-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-35959-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-35959-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-35959-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-46721-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 46721, for 17.10.in-addr.arpa.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
17.10.in-addr.arpa. IN DNSKEY 257 3 15 VDMrARYpniY1/Cv+SNVc/X8RKqdfixTF4Oy8KvhNw7E=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-46721-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-46721-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-46721-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-46721-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-32318-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: iSVkLKJt2PIPdpnnjDhfRlEK5wAqP4iwe8FEiMPE/5U=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-32318-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-32318-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-32318-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-32318-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-43575-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: +RejUif2h7wBX/4QVP2rr/0mSFGjYuVVxUNfvGYE+M0=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-43575-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-43575-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-43575-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-43575-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-32318-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 32318, for epl-infra.net.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
epl-infra.net. IN DNSKEY 256 3 15 Vak8RFVVgr/2YvVPGLtEOlUt8eMoruufwf9KInsX5Os=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-32318-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-32318-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-32318-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-32318-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-43575-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 43575, for epl-infra.net.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
epl-infra.net. IN DNSKEY 257 3 15 V6TM5tDL/SRWUs0b/Nm2YP5Y9IMowBByoAKFEXUlFro=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-43575-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-43575-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-43575-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-43575-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23158-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: AR+tm3umS8SJ10C6Dv6qHQC8q+4+vkvBCDqU8FUhm/Y=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23158-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23158-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-23158-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23158-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-26944-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 70Q401FSai0zYxSCVveAbwfx2rMoq/ISMb60iI7/VOc=
Created: 20240112120947
Publish: 20240112120947
Activate: 20240112120947
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-26944-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-26944-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-26944-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-26944-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23158-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 23158, for us-west.epl-infra.net.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
us-west.epl-infra.net. IN DNSKEY 256 3 15 rLMjD9jwJpRVy6WhhUTJhLJqdowHatrAgX7PB/Uu7BE=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-23158-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23158-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-23158-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-23158-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-26944-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 26944, for us-west.epl-infra.net.
; Created: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Publish: 20240112120947 (Fri Jan 12 14:09:47 2024)
; Activate: 20240112120947 (Fri Jan 12 14:09:47 2024)
us-west.epl-infra.net. IN DNSKEY 257 3 15 RL0T0yIizdXnycf6HsTffd6TlD+sHMVn+XVRncDgflU=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-26944-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-26944-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-26944-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-26944-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
ed753f50-b6a7-4a9e-95e9-43abcde31f25
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

2f6+H0CwfQ4hkVnQtE3GMhOGYSAUj3Gj
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIE3umXJeZfTQP2lu/52b6ofjvybzjr5qoLAeCYtQm0HgoAoGCCqGSM49
AwEHoUQDQgAEKjrq0tqLlNQdGTN+923PfbL5uv8kzaxyXzyEcr7HgOf1ETzXiQwW
x2Cps0LxRSil1dzxJwhs+fiUhJJdqS3zyg==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

x7rqFYKsKAH/iARaL+BBK7QJB4Ik
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEILT7tH0Qln0ZFpMVakvneBge6FUtFy2pAIJtOttTqxaVoAoGCCqGSM49
AwEHoUQDQgAE+rVDiA0NmCUiKHseDZ7eZl0eGuH5IoI5zNXXiQRkJgM4VK4OdZI+
C1xTgenalMVKdEhzQIcDqMsZMfuLzzcspg==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

ssDviIU5pH92TRmBxEL/SiUvqLH94Hlv
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

...
gglsb2NhbGhvc3SHBH8AAAEwCgYIKoZIzj0EAwIDSAAwRQIhAObHSbd6BXXzV/Lz
ZUWhxSBIYkWGyCJ1CkaqoR+Aw4W2AiBx0OkMhDdWxrCiXuLonUn/Ja2fIX116Ln5
mdsWpki2hg==
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIFZwFl2sykY38TYIcADaGHOSSqKDc9dd8aKZ1CpDmA95oAoGCCqGSM49
AwEHoUQDQgAEYhqr2qwhiTpXrwqRH6tHqha0yErmdO/QtBOte78cIRp5X/lDPMhz
m/edvs5taUHEhtrMQU9kBb7f629WB0PnLg==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
OA+DfJ0hOMJxMtT84qqQ0uCXLhdbp2EPEonf3RlCCGU=
LilBoiPeepLikesBenzTruck
...
# us-west.epl-infra.net.	IN	DS	26944 15 2 279CA6E87D9B55441961553BAA1EC407AB6E47D7AF54A392583E6C664078AFD1
#
maybe_update_dns_file  /run/named/private-epl-infra.net.zone
# $TTL 3600
...
zpool import -af
provision_zfs_dataset rpool acme /var/lib/acme 128k on no 4IN8AQL3qayEnK0IwyDuhuNXv9ADdlmUFADY2LyJPV
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool minio-docker-d /srv/volumes/minio-docker-d 1M on yes tp8EcS6CpX0eEvuQzvrSRXRsL1IVXIsN4Q6OO7TLNi
provision_zfs_dataset rpool mon-am /srv/volumes/mon-am 4k on yes S4YQMKQd8k6BYpCKQ8mbbiFlkFfkJbEoKUEwAp7uOH
provision_zfs_dataset rpool nats1 /srv/volumes/nats1 4k on yes 2yEjP0NcAXIZl7EX3yEJuzdtPqrWoudQyA7vc9h6Hn
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no rMSkG3ajLYF7isyfSGk7eud8fu37ygy2SX3irV4ttu
provision_zfs_dataset rpool vault /var/lib/vault 4k on no SjGLlHRrIYqzniWf9BP03dVq0xRAC1QC59LCsWYnM4
...
{ ... }:
{
    ...
    nix.settings = {
      ...
      trusted-public-keys = [
        "epl-nix-cache:Htcp8+94uGb+3NePWrhmCizroI6OTlxf5du0jTwIRWI="
      ];
...
# NIX REGION firewall END
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-d.dc1.us-west.gcloud-single-dc:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-d.dc1.us-west.gcloud-single-dc: \w\a\]\u@server-d.dc1.us-west.gcloud-single-dc:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-d.dc1.us-west.gcloud-single-dc:\u:\w\007\]$PS1"
       fi
     fi
   '';
  ...
  fileSystems."/boot" = {
    ...
    services.bind =
    {
        ...
        extraConfig = ''
          trust-anchors {
  epl-infra.net. initial-key 257 3 15 "V6TM5tDL/SRWUs0b/Nm2YP5Y9IMowBByoAKFEXUlFro=";
  us-west.epl-infra.net. initial-key 257 3 15 "RL0T0yIizdXnycf6HsTffd6TlD+sHMVn+XVRncDgflU=";
  10.in-addr.arpa. initial-key 257 3 15 "dtT6lBJJpxBrviwLFIVVHZGySeT/ApDPkgCqJn5RGNM=";
  17.10.in-addr.arpa. initial-key 257 3 15 "VDMrARYpniY1/Cv+SNVc/X8RKqdfixTF4Oy8KvhNw7E=";
  in-addr.arpa. initial-key 257 3 15 "VeoRGYALJKcKZnAdN29GTZuOWwfGdrDPBDyNNOnR47c=";
};
...
# peers
wg set wg0 peer "vX9XbA/ieTpxHFCVHG6zC8QzmxfggrOkmjC0S+HngAI=" allowed-ips "172.21.7.254/32"
...
# NIX REGION l3_vpn_hop_interface START
  ...
  systemd.services.vpn-gre-tunnel = {
    ...
    script = ''
        ip tunnel add vpnGre mode gre local 10.17.1.11 key 17
        ip addr add 10.17.129.11/17 dev vpnGre
        ...
        ip neighbor add 10.17.129.12 lladdr 10.17.1.12 dev vpnGre
...
# NIX REGION epl_nft_rules_l3-vpn-hop-address-translation START
        ...
        chain PREROUTING {
            ...
            ip daddr 10.17.1.12 ip saddr 10.0.0.0/8 ip daddr set 10.17.129.12;
            ip saddr 10.17.129.12 ip saddr set 10.17.1.12;
        }
...
cp -pu /run/keys/K10-in-addr-arpa--015-21012-private /run/dnsseckeys/K10.in-addr.arpa.+015+21012.private
cp -pu /run/keys/K10-in-addr-arpa--015-28404-private /run/dnsseckeys/K10.in-addr.arpa.+015+28404.private
cp -pu /run/keys/K10-in-addr-arpa--015-21012-key /run/dnsseckeys/K10.in-addr.arpa.+015+21012.key
cp -pu /run/keys/K10-in-addr-arpa--015-28404-key /run/dnsseckeys/K10.in-addr.arpa.+015+28404.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-35959-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+35959.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-46721-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+46721.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-35959-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+35959.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-46721-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+46721.key
cp -pu /run/keys/Kepl-infra-net--015-32318-private /run/dnsseckeys/Kepl-infra.net.+015+32318.private
cp -pu /run/keys/Kepl-infra-net--015-43575-private /run/dnsseckeys/Kepl-infra.net.+015+43575.private
cp -pu /run/keys/Kepl-infra-net--015-32318-key /run/dnsseckeys/Kepl-infra.net.+015+32318.key
cp -pu /run/keys/Kepl-infra-net--015-43575-key /run/dnsseckeys/Kepl-infra.net.+015+43575.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-23158-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+23158.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-26944-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+26944.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-23158-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+23158.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-26944-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+26944.key
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"e55eb38372f4651e7ea41e6cddcac1c6801d8fc4fe23564155e3fc87d69d751f\",hostname=\"server-d\"} $BOOT_TIME
" > $METRICS_FILE.tmp
