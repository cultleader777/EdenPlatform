...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
NhoB9Iv850WySz5meKzUpjEOgBvhiZ2sJqadQiM05Ro=
LilBoiPeepLikesBenzTruck
...
provision_zfs_dataset tank pgtest1 /srv/volumes/pgtest1 4k on yes SZMBkvjDJiPRz6wEhUQ34kgTQMlYHdoB6KyZHeZHuc
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-g.dc3.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-g.dc3.us-west.multi-dc-region: \w\a\]\u@server-g.dc3.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-g.dc3.us-west.multi-dc-region:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
if [[ ! $( timeout 0.5s dig @127.0.0.1 server-g.us-west.epl-infra.net +dnssec +short ) ]]
then
