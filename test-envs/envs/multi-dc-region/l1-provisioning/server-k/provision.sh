...
host = "server-k.us-west.epl-infra.net"
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
MeLlVYXq+NDzWXFPN2YbpFaalkFCVxYFE559T7TLqbc=
LilBoiPeepLikesBenzTruck
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-k.dc1.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-k.dc1.us-west.multi-dc-region: \w\a\]\u@server-k.dc1.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-k.dc1.us-west.multi-dc-region:\u:\w\007\]$PS1"
       fi
     fi
   '';
     ...
     systemd.services.l1-fast-agent = {
       ...
       ''
         ...
         while ! ${pkgs.consul}/bin/consul kv get epl-l1-plans/server-k
         do
           sleep 7
         done

         ${pkgs.consul}/bin/consul watch \
           -type=key -key=epl-l1-plans/server-k \
           ${l1Checker}/checker \
...
if [[ ! $( timeout 0.5s dig @127.0.0.1 server-k.us-west.epl-infra.net +dnssec +short ) ]]
then
