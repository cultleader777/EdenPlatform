...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
{
  "acl": {
    ...
    "tokens": {
      ...
      "initial_management": "6af6c4bb-fdc6-4ac7-ba9d-40876e5c955e"
    }
  },
  "addresses": {
    "dns": "127.0.0.1",
...
client {
  ...
  host_volume "am-default" {
    path = "/srv/volumes/am-default"
    read_only = false
  }
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
nRsIBg73WpBcpK0c75ur+5j7Wgbmk00EFqR9kyAHGEY=
LilBoiPeepLikesBenzTruck
...
# NIX REGION secret_value_K10-in-addr-arpa--015-55597-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: jWNCZFbK/GZXkHUT0iEEilSi+UUQJh7Nda/WtDubTvY=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-55597-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-55597-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-55597-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-55597-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-56550-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: zYCS04QI7DnvJvjHO6IcwKVugaDTPvKeAbZWltgs2v4=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-56550-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-56550-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-56550-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-56550-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-55597-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 55597, for 10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
10.in-addr.arpa. IN DNSKEY 256 3 15 BMaTz4iTj6trXLMLzsQ63TUhNIjLD8rFB7SK1FQPFik=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-55597-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-55597-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-55597-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-55597-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-56550-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 56550, for 10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
10.in-addr.arpa. IN DNSKEY 257 3 15 bikQayWtJU1UobatHlj+k6BqIwJ3HL5wDGmFJCYGNn8=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-56550-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-56550-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-56550-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-56550-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-51849-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: /aXzG0FcxhvppRwvVJCaWGKS7uTxcnTzDbqcMBDyO14=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-51849-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-51849-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-51849-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-51849-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-65209-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: Bf9A7Zz4woODhxDSl6ZWH2j+poT+l9TOaMTZxYOfcJ8=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-65209-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-65209-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-65209-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-65209-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-51849-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 51849, for 17.10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
17.10.in-addr.arpa. IN DNSKEY 256 3 15 IASjKHgpWIHdPtnl9n1wsCmwXCeJZeAgOq1LE42qEYc=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-51849-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-51849-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-51849-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-51849-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-65209-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 65209, for 17.10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
17.10.in-addr.arpa. IN DNSKEY 257 3 15 2g860cxdp4pgcPiCUJGjPkyKv1eTvln0f3y9PJOzWxs=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-65209-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-65209-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-65209-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-65209-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-36290-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: Ue4exb45v9zbeYsj18rMsKsrzFsa+04grr1AxBeNShY=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-36290-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-36290-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-36290-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-36290-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-52333-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: B9KGy5GhUHSmT0uQqhRo8lQvbJKBYabfYeJZ/Xnb+qI=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-52333-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-52333-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-52333-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-52333-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-36290-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 36290, for 18.10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
18.10.in-addr.arpa. IN DNSKEY 256 3 15 1nSTdubDdkFL1ScXt0eapdTqZr8StLA25+VBJUOukpo=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-36290-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-36290-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-36290-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-36290-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-52333-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 52333, for 18.10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
18.10.in-addr.arpa. IN DNSKEY 257 3 15 l+q/pFWjaFsDbBP6CfO9ISqlq5KuWcnx3Ot1Q+OCUB8=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-52333-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-52333-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-52333-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-52333-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K19-10-in-addr-arpa--015-53678-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: OsNZ7KbpzD+wK5ijt/yn+buOf3A7RIO2DWnnjQyAllQ=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K19-10-in-addr-arpa--015-53678-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-53678-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K19-10-in-addr-arpa--015-53678-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-53678-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K19-10-in-addr-arpa--015-03592-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: pHdi74f1EIWf1e1NpyGarucyuH6VK8OVb1crKNvZJ0g=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K19-10-in-addr-arpa--015-03592-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-03592-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K19-10-in-addr-arpa--015-03592-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-03592-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K19-10-in-addr-arpa--015-53678-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 53678, for 19.10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
19.10.in-addr.arpa. IN DNSKEY 256 3 15 xIY1/oftvuAKSGRna+Uajo1195VnMaomo6DhHAcndOE=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K19-10-in-addr-arpa--015-53678-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-53678-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K19-10-in-addr-arpa--015-53678-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-53678-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K19-10-in-addr-arpa--015-03592-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 3592, for 19.10.in-addr.arpa.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
19.10.in-addr.arpa. IN DNSKEY 257 3 15 nG0O/J7rtK63VHqsf8O1Pmon52Cjo40gTgLVoDs9wsM=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K19-10-in-addr-arpa--015-03592-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-03592-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K19-10-in-addr-arpa--015-03592-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K19-10-in-addr-arpa--015-03592-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-39010-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: b2KQbk8hz25hZdzS2l+GD5xOZhVX1K781VwZWMky3oo=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-39010-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-39010-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-39010-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-39010-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-57684-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 3rGyP5bvGDLN9iVMd5j40fhhOIaOcj50EJCUWUTxbLY=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-57684-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-57684-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-57684-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-57684-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-39010-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 39010, for epl-infra.net.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
epl-infra.net. IN DNSKEY 256 3 15 OcaQ+jI9Wuy0I0M9YAY6N0Q//WYCB7E7aMxRJeaVFVo=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-39010-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-39010-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-39010-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-39010-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-57684-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 57684, for epl-infra.net.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
epl-infra.net. IN DNSKEY 257 3 15 KTwKhuMIFItc9B2XscE5GxKhbosPHJkEjqay2xUNzKk=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-57684-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-57684-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-57684-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-57684-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-29618-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: ZjUGSch3iLCBwxLnMZ5WuG4rXn3Sojr0EPFB2uqmaRA=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-29618-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-29618-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-29618-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-29618-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-42144-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: yjq0DuWHnBfdegB6tI8QU1ZIO60lrCGWCX5H/acPn1o=
Created: 20230817142942
Publish: 20230817142942
Activate: 20230817142942
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-42144-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-42144-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-42144-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-42144-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-29618-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 29618, for us-west.epl-infra.net.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
us-west.epl-infra.net. IN DNSKEY 256 3 15 SoCheleYcw37pKdglrt02Tbx3EkNObDZTv1FtXGaMs0=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-29618-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-29618-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-29618-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-29618-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-42144-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 42144, for us-west.epl-infra.net.
; Created: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Publish: 20230817142942 (Thu Aug 17 17:29:42 2023)
; Activate: 20230817142942 (Thu Aug 17 17:29:42 2023)
us-west.epl-infra.net. IN DNSKEY 257 3 15 BfHDq18a7xXIZOI67MOPmVQNp2SYCh4oX98mXnEKuNk=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-42144-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-42144-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-42144-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-42144-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIHDziMYfYidBPLiqJZQCovLFINfV2PubY051Rq+j5Z8OoAoGCCqGSM49
AwEHoUQDQgAEguwi5Oujpo1HNccf9BVe95NudvrehddOrAObjyBac9Py1zDCbTh7
49fOVskfP+EOd5EfLlZiV6cudvlTS03nmw==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
6af6c4bb-fdc6-4ac7-ba9d-40876e5c955e
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
7504fe6a-f649-40be-b6c3-d7bb3676bdd2
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
4331da8d-fd4c-4380-bd8a-32f94346da04
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
6c863dd6-9ef7-431d-8245-5d83430fc1da
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
5358b53d-6cc8-4ef8-af01-e70fb529a366
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
16f61e0e-606d-4072-8794-61f0a34cb2ae
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
e3dc00ce-8dcc-488d-af05-90d50c72b3b9
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
d8e793ce-d098-48dc-b45a-43a342948cd8
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

...
c3Vsgglsb2NhbGhvc3SHBH8AAAEwCgYIKoZIzj0EAwIDSAAwRQIhAJwYkOIqVreG
/NkEXaFT5wQFHhQgBqd4dnsiEIra5Yf+AiAC+sgUuqaQWcSqFMB6SXxn/cdr3Qdf
w4TzTgvsLeZ+hA==
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIFWMBkFJpTsutd0Mf+8oueGP4x3wF2E/lYdxnT3XaxyFoAoGCCqGSM49
AwEHoUQDQgAEzmDEmLq9adJ2kzyqImmplDKd8cIIYy3UXrf56HmTa1AbjnX5ckzp
n/Mhm7Eblp5c+gXYYXtlJU+YBBsSFRBnSg==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

03p5yH20us1Zusr5ROUWFpP9X3ke
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEICb86/tx5wejdf9p3R5YxR7ouWkMcc02eESB3pwgFbhXoAoGCCqGSM49
AwEHoUQDQgAE1pATTfVeiE6SVPstZfflwwsHM7pxB+hbxwtNMm6goiZ1XjfVeRW8
BCuI4PjVeCcjk4C6dzQmE3pDJpB6VoBsUw==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
f2b92f8e-803d-4df5-96fb-9f5913dadab0
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
QHjKpbnNR2yPHIhEHYfh/S0vXfobYbIiD2TW4dqY3lQ=
LilBoiPeepLikesBenzTruck
...
# ns1.epl-infra.net.	IN	A	10.17.0.10
# ns2.epl-infra.net.	IN	A	10.18.0.10
...
# ns1.us-west.epl-infra.net.	IN	A	10.17.0.10
# ns2.us-west.epl-infra.net.	IN	A	10.18.0.10
# ns3.us-west.epl-infra.net.	IN	A	10.19.0.10
# us-west.epl-infra.net.	IN	DS	42144 15 2 DAC4C36B9EAA0D01CD2D5FAA02DACE63FE53595E7EB6A9EE1D6F33C27461BF32
#
maybe_update_dns_file  /run/named/private-epl-infra.net.zone
# $TTL 3600
...
#
# 10.0.17.10.in-addr.arpa.	IN	PTR	ns1.epl-infra.net.
# 10.0.18.10.in-addr.arpa.	IN	PTR	ns2.epl-infra.net.
...
maybe_update_dns_file  /run/named/private-10.in-addr.arpa.zone
# $TTL 3600
...
# server-b.us-west.epl-infra.net.	IN	A	10.17.0.13
...
maybe_update_dns_file  /run/named/private-us-west.epl-infra.net.zone
# $TTL 3600
...
# 17.10.in-addr.arpa.	IN	NS	ns3.us-west.epl-infra.net.
#
# 10.0.17.10.in-addr.arpa.	IN	PTR	ns1.us-west.epl-infra.net.
# 13.0.17.10.in-addr.arpa.	IN	PTR	server-b.us-west.epl-infra.net.
maybe_update_dns_file  /run/named/private-17.10.in-addr.arpa.zone
# $TTL 3600
...
# 18.10.in-addr.arpa.	IN	NS	ns3.us-west.epl-infra.net.
#
# 10.0.18.10.in-addr.arpa.	IN	PTR	ns2.us-west.epl-infra.net.
# 11.0.18.10.in-addr.arpa.	IN	PTR	server-d.us-west.epl-infra.net.
maybe_update_dns_file  /run/named/private-18.10.in-addr.arpa.zone
# $TTL 3600
...
maybe_update_dns_file  /run/named/private-19.10.in-addr.arpa.zone
# $TTL 3600
# epl-infra.net.	IN	SOA	ns1.epl-infra.net. epl-infra.net. (
#  SERIAL_TO_REPLACE ; Serial
#  3600 ; Refresh
...
# epl-infra.net.	IN	NS	ns2.epl-infra.net.
# epl-infra.net.	IN	NS	ns3.epl-infra.net.
# ns1.epl-infra.net.	IN	A	77.77.77.10
# ns2.epl-infra.net.	IN	A	77.77.77.12
# ns3.epl-infra.net.	IN	A	77.77.77.14
...
maybe_update_dns_file  /run/named/public-epl-infra.net.zone
# $TTL 3600
...
#
# 10.77.77.77.in-addr.arpa.	IN	PTR	ns1.epl-infra.net.
...
maybe_update_dns_file  /run/named/public-in-addr.arpa.zone
...
zpool import -af
provision_zfs_dataset rpool acme /var/lib/acme 128k on no o1w0h08iW4SEkpeYB7mgJQ2MKk9c4RY4f5IygwqmhD
provision_zfs_dataset rpool am-default /srv/volumes/am-default 4k on yes bkAli7QNUZUBhGqCBzBTn778fu3t00h7whigYAIkUo
provision_zfs_dataset rpool consul /var/lib/consul 4k on no oH04cm4h657qKNaDdpbAQw9SzKESvAcDwtE1Q4weNA
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool minio-docker-a /srv/volumes/minio-docker-a 1M on yes AFrQRzU7KK8bZk8Rcwm2OOfZGU8RSLQ7GwNf3Hbuw9
provision_zfs_dataset rpool mon-default /srv/volumes/mon-default 4k on yes ygQsBMoaYBSccNUK2pOxZytLPFz2QY24rqMA93g6sd
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no fuzk0o3nrf04tksutNkPsF4mBlq2AuSztqLATWkT2X
provision_zfs_dataset rpool pgtest1 /srv/volumes/pgtest1 4k on yes LwfrqwJsdoBLr2zWTPrIPAXDN6GkIEscwLLqBrA4hp
...
# NIX REGION epl_nft_rules_epl-internet-fw START
       ...
       chain EPL_INTERNET_FIREWALL {
           ...
           iifname enp2s0 ip saddr != { 10.0.0.0/8, 172.21.0.0/16 } ip daddr != { 77.77.77.10/32 } drop comment "Disallow traffic from internet to internal networks";
       }
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-a.dc1.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-a.dc1.us-west.multi-dc-region: \w\a\]\u@server-a.dc1.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-a.dc1.us-west.multi-dc-region:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
wg set wg0 peer "RYr316dq4Gke5Hpql5HKaZB4DYlgcejUD5q28GEzNkI=" allowed-ips "172.21.7.14/32,10.19.0.0/16" endpoint "77.77.77.14:51820"
  ...
  services.frr.ospf = {
      ...
      config = ''
        ...
        interface enp1s0
          ...
          ip ospf message-digest-key 12 md5 tBAbIDhsOOsVGRmu
  ...
  services.frr.bgp = {
      ...
      config = ''
        ...
        router bgp 64529
          ...
          neighbor 10.17.0.13 password K1R0yi1RUSobddXmU7pCUamxY4JVA6CaV4BRXm0Loj
          neighbor 10.17.0.13 bfd
          neighbor 172.21.7.12 remote-as 64530
          neighbor 172.21.7.12 password gYHQS5MS8Xvgte3X57rPIGRDS6OATfirNYx7xQ8Znr
          neighbor 172.21.7.12 bfd
          neighbor 172.21.7.14 remote-as 64531
          neighbor 172.21.7.14 password Dfce16fxmGNGfmm3axApRZJA8n9sgX7krZbhiKTzpw
...
# NIX REGION keepalived END
    ...
    networking.interfaces.enp2s0.ipv4.addresses = [
      { address = "77.77.77.10"; prefixLength = 24; }
...
cp -pu /run/keys/K10-in-addr-arpa--015-55597-private /run/dnsseckeys/K10.in-addr.arpa.+015+55597.private
cp -pu /run/keys/K10-in-addr-arpa--015-56550-private /run/dnsseckeys/K10.in-addr.arpa.+015+56550.private
cp -pu /run/keys/K10-in-addr-arpa--015-55597-key /run/dnsseckeys/K10.in-addr.arpa.+015+55597.key
cp -pu /run/keys/K10-in-addr-arpa--015-56550-key /run/dnsseckeys/K10.in-addr.arpa.+015+56550.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-51849-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+51849.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-65209-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+65209.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-51849-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+51849.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-65209-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+65209.key
cp -pu /run/keys/K18-10-in-addr-arpa--015-36290-private /run/dnsseckeys/K18.10.in-addr.arpa.+015+36290.private
cp -pu /run/keys/K18-10-in-addr-arpa--015-52333-private /run/dnsseckeys/K18.10.in-addr.arpa.+015+52333.private
cp -pu /run/keys/K18-10-in-addr-arpa--015-36290-key /run/dnsseckeys/K18.10.in-addr.arpa.+015+36290.key
cp -pu /run/keys/K18-10-in-addr-arpa--015-52333-key /run/dnsseckeys/K18.10.in-addr.arpa.+015+52333.key
cp -pu /run/keys/K19-10-in-addr-arpa--015-53678-private /run/dnsseckeys/K19.10.in-addr.arpa.+015+53678.private
cp -pu /run/keys/K19-10-in-addr-arpa--015-03592-private /run/dnsseckeys/K19.10.in-addr.arpa.+015+03592.private
cp -pu /run/keys/K19-10-in-addr-arpa--015-53678-key /run/dnsseckeys/K19.10.in-addr.arpa.+015+53678.key
cp -pu /run/keys/K19-10-in-addr-arpa--015-03592-key /run/dnsseckeys/K19.10.in-addr.arpa.+015+03592.key
cp -pu /run/keys/Kepl-infra-net--015-39010-private /run/dnsseckeys/Kepl-infra.net.+015+39010.private
cp -pu /run/keys/Kepl-infra-net--015-57684-private /run/dnsseckeys/Kepl-infra.net.+015+57684.private
cp -pu /run/keys/Kepl-infra-net--015-39010-key /run/dnsseckeys/Kepl-infra.net.+015+39010.key
cp -pu /run/keys/Kepl-infra-net--015-57684-key /run/dnsseckeys/Kepl-infra.net.+015+57684.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-29618-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+29618.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-42144-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+42144.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-29618-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+29618.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-42144-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+42144.key
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"46b583cc1f3cd085b79e0f353fe3deecfae4c18317234196836b7c9662c979a0\",hostname=\"server-a\"} $BOOT_TIME
" > $METRICS_FILE.tmp
  ...
  echo '
epl_l1_provisioning_expected_hash{hash="46b583cc1f3cd085b79e0f353fe3deecfae4c18317234196836b7c9662c979a0",hostname="server-a"} 1
epl_l1_provisioning_expected_hash{hash="5ece141f247325d73f6c8598ce69bcbc67df15d2b457cd599330f02c21d991d6",hostname="server-b"} 1
epl_l1_provisioning_expected_hash{hash="df9989d3f64377301c654c24f50c72b9bf3319d10f55f5b7bf4c41e5fae12dbe",hostname="server-c"} 1
epl_l1_provisioning_expected_hash{hash="6a270efa0ca945b0b48304c00de8baee7aa8ca6e4ed955dcb81fe913aa192469",hostname="server-d"} 1
epl_l1_provisioning_expected_hash{hash="11a1b380297d38c1043fb992697aa3b584cecf725c30aba807356c61016093cf",hostname="server-e"} 1
epl_l1_provisioning_expected_hash{hash="ba973c1d37ef5b9fe1cca2f9e9988d78d30429e04e478a913bd1d34388ab553a",hostname="server-f"} 1
