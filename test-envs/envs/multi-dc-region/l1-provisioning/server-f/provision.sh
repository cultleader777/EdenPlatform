...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
QJzZZCLOToSfEQQr+H1IjeezloNGczhPt8K5OFdWN/4=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

...
gglsb2NhbGhvc3SHBH8AAAEwCgYIKoZIzj0EAwIDSAAwRQIgTzNrRgi0mubRuzOh
N18tl6GK/nsrJjkqiEQNsqI4UdACIQCQY839y9irfwHTo0drYfTvEaab2f6MLVed
bgC2wfu7pA==
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIGeEqaGyce++Eohl4sCCz87tvNnneEdwPBnOYaXiGXtXoAoGCCqGSM49
AwEHoUQDQgAE/GUr029dTgHu42C7LM/L6RXyfc520mdLoAJeejXaCwAWa0iuTRxK
GL6h9Wfp9JQgALfstlR3X7hS5J4WxfEcig==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
4AndwOdavxgVRDirZK3hjx2qkYpBUrWFVWDl8h/g3Gk=
LilBoiPeepLikesBenzTruck
...
provision_zfs_dataset rpool minio-docker-d /srv/volumes/minio-docker-d 1M on yes gHec1BCOKBNX4DQwh4C6V8GfT6ItXesw8lxwdeorxU
provision_zfs_dataset rpool nats1 /srv/volumes/nats1 4k on yes mqahGzibCDoFPcVqGmYIvszyYxDrJQHRvTRspmFarf
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no jFeOuLjfKQyqFDApQvxlmo8rRLGR0yroj4QjcU0G3P
provision_zfs_dataset rpool vault /var/lib/vault 4k on no nf6oDW2AMhoLeVhxc9rjaacYVafq2jagC4Uy9lYxMs
...
# NIX REGION epl_nft_rules_epl-internet-fw START
       ...
       chain EPL_INTERNET_FIREWALL {
           ...
           iifname enp2s0 ip saddr != { 10.0.0.0/8, 172.21.0.0/16 } ip daddr != { 77.77.77.15/32 } drop comment "Disallow traffic from internet to internal networks";
       }
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-f.dc3.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-f.dc3.us-west.multi-dc-region: \w\a\]\u@server-f.dc3.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-f.dc3.us-west.multi-dc-region:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
wg set wg0 peer "AO3O917iwQeh+EaHmAKsg3ruezjOjix/cGsvl7ajZVc=" allowed-ips "172.21.7.13/32,10.18.0.0/16" endpoint "77.77.77.13:51820"
  ...
  services.frr.bgp = {
      ...
      config = ''
        ...
        router bgp 64531
          ...
          neighbor 10.19.0.10 password b24p05HIWXpFAaOOxcBHVjV5k0kyWZOh749P3V8bwp
          neighbor 10.19.0.10 bfd
          neighbor 172.21.7.11 remote-as 64529
          neighbor 172.21.7.11 password 5tsWLbuERm4LT2DWgIZZ3YJx9LJzPpwts8pnqtx5um
          neighbor 172.21.7.11 bfd
          neighbor 172.21.7.13 remote-as 64530
          neighbor 172.21.7.13 password jRmc3Idvt3HJjeHXrtxmVFMovg6OeS2d9mft9GlmMp
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"ba973c1d37ef5b9fe1cca2f9e9988d78d30429e04e478a913bd1d34388ab553a\",hostname=\"server-f\"} $BOOT_TIME
" > $METRICS_FILE.tmp
