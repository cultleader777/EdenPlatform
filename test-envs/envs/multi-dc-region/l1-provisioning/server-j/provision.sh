...
/run/current-system/sw/bin/ip route add 10.17.0.1/32 via 10.17.1.1
...
host = "server-j.us-west.epl-infra.net"
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
ISxHP2tzqcvBjoFfeAeoYpE5TqH1ejYjod9rGapyRI8=
LilBoiPeepLikesBenzTruck
...
provision_zfs_dataset tank pgtest1 /srv/volumes/pgtest1 4k on yes cNpqIMpMjBnCOJ9zKSQUURReh2laMPzH55joguRYsh
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-j.dc1.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-j.dc1.us-west.multi-dc-region: \w\a\]\u@server-j.dc1.us-west.multi-dc-region:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-j.dc1.us-west.multi-dc-region:\u:\w\007\]$PS1"
       fi
     fi
   '';
     ...
     systemd.services.l1-fast-agent = {
       ...
       ''
         ...
         while ! ${pkgs.consul}/bin/consul kv get epl-l1-plans/server-j
         do
           sleep 7
         done

         ${pkgs.consul}/bin/consul watch \
           -type=key -key=epl-l1-plans/server-j \
           ${l1Checker}/checker \
...
# NIX REGION frr_static_routes START
  ...
  environment.etc."frr/staticd.conf".text = ''
        !
        ip route 10.17.0.1/32 10.17.1.1
...
if [[ ! $( timeout 0.5s dig @127.0.0.1 server-j.us-west.epl-infra.net +dnssec +short ) ]]
then
