...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
wfeyCh8aFyUeUd6vhhCS0Mfdkk0pNQKkYDPvR8X7VJg=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----
MIICxTCCAmugAwIBAgIRANyxupCiVIUYQdFr01rm4eIwCgYIKoZIzj0EAwIwgbkx
...

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEII7TTuiH9beljDK/kn//K7+u3BKGQKX072WAb4Y0byozoAoGCCqGSM49
AwEHoUQDQgAET1G/uxwmsmz7IJK8gB9Nd40xFIEl+4X6ZELZ5PwWMuxwCg6agQS/
no9u8Ho21NMxKFZg4666ORBNOEcxT7DOcg==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
zpool import -af
zpool list -H -o name | grep 'extra' || zpool create extra -m none -o autoexpand=on -o ashift=12 mirror vdc vdd mirror vde vdf spare vdg cache vdh log mirror vdi vdj
...
then
    zpool add extra mirror vde vdf
fi
...
then
    zpool add extra log vdi
fi
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '1,/^\s+logs/ d' | sed -E '/^\s+cache/q' | sed -E '/^\s+spares/q' | grep -E '\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "2" ];
then
    zpool add extra log vdj
fi
...
if [ "$ZPOOL_DISK_COUNT" -lt "1" ];
then
    zpool add extra cache vdh
fi
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '1,/^\s+spares/ d' | grep -E '^\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "1" ];
then
    zpool add extra spare vdg
fi
zpool online -e extra vdc
zpool online -e extra vdd
zpool online -e extra vde
zpool online -e extra vdf
zpool online -e extra vdi
zpool online -e extra vdj
provision_zfs_dataset extra bigone /srv/volumes/bigone 4k on yes VmROdcLgkposlXUSkowzff0nmcc8l7lwrW9IMls2Kk
provision_zfs_dataset rpool consul /var/lib/consul 4k on no klwOzeBwkN1vqbB2Eq8ffUq8VG5ZJJErjEWq17lKQt
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool mon-am /srv/volumes/mon-am 4k on yes Y2Umb4vgCzAqy8CpY8yDn0QOGS20O4NSNySQa5ll1q
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no jokVyx3gNYqU9o6iMdveaH7Uim7g1iSmj4MwhzMvGp
provision_zfs_dataset rpool pgtest1 /srv/volumes/pgtest1 4k on yes qfWnBR76w8wG6G3Gnw2tdoYpN8d4C9bWK5N60oACi2
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-a.dc1.us-west.single-dc:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-a.dc1.us-west.single-dc: \w\a\]\u@server-a.dc1.us-west.single-dc:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-a.dc1.us-west.single-dc:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"2db86dc4fd18cb7c93fcdd2c7bc9833346d93e87c33a85e0918733189aafe148\",hostname=\"server-a\"} $BOOT_TIME
" > $METRICS_FILE.tmp
  ...
  echo '
epl_l1_provisioning_expected_hash{hash="2db86dc4fd18cb7c93fcdd2c7bc9833346d93e87c33a85e0918733189aafe148",hostname="server-a"} 1
epl_l1_provisioning_expected_hash{hash="d79b7452e6735499143f1a719274b4222abaf72175ea6f2119cc9537666b2e72",hostname="server-b"} 1
epl_l1_provisioning_expected_hash{hash="21eb422b9e027ab2c88649239917e268f9c958cb7742622befc438464e23f7e6",hostname="server-c"} 1
epl_l1_provisioning_expected_hash{hash="84f8d66c656c8ae90feb66fcb9d0ac0cc42f06cf3fb27f925adfd71f6e069d3c",hostname="server-d"} 1
epl_l1_provisioning_expected_hash{hash="2bbf5f3f10a8283a6391367ed4bf0dd286f01cece8859f23cb663afda11b02eb",hostname="server-e"} 1
epl_l1_provisioning_expected_hash{hash="77c2aab85379862eeb504378dec512daa5043754837a56684b7838ff8a54e6d4",hostname="server-f"} 1
