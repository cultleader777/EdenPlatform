...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
{
  "acl": {
    ...
    "enabled": true,
    "tokens": {
      "agent": "fb8298ae-e2ce-4da0-939d-8d6b84d4c7dd",
      "default": "475b6cd0-e040-4d93-a233-470f4a2566b9"
    }
  },
  "addresses": {
    "dns": "127.0.0.1",
...
consul {
    ...
    cert_file = ""
    key_file = ""
    token = "581ad3af-950b-49c3-b0be-48cc54c2bdfe"
...
vault {
    ...
    namespace = ""
    token = "hvs.CAESIJU5dPQX0pAEeGHyNLJtb-X2xCaxo0PKEc820Lyo6S_xGh4KHGh2cy5UQ2FNMEFhVlNZQ0FLbzB3MUtQdXB1aVE"
...
client {
  ...
  meta = {
    ...
    "lock_epl-mon-server-d-default" = "1"
...
server {
    ...
    encrypt = "ciGBWkfvdi8EBrakPDtdYNMBnxzmkdGLR4Z2DMVkZv4="
...
service_registration "consul" {
  ...
  token = "54a711d6-cbc6-4084-aa93-5431dde0fcdd"
}
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
LpXycRh955C0xZzR47s3X1IQJWbSH2czdQnAE5Kfd2w=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
JZzKafLUtgqL4x6FNWGYSSx/+1gX7vxcYm/N35+Vomc=
LilBoiPeepLikesBenzTruck
...
MHcCAQEEICtR0n4LIRh1gTZg1OaerovFe/jk0hHNgdirjOflkvcHoAoGCCqGSM49
AwEHoUQDQgAEEqCDKLlz2H3z4KWySgR7GKT9y/CsVNVDh+D4/222F9+hXuqzOrrv
nNkrbHy+A7oUQPvmkkqLs6pfO8sVBO/EfA==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
# NIX REGION secret_value_K10-in-addr-arpa--015-62402-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: AHc3z21VwypqWYT+K3efUvM5lbz8wJ0oqFxlyS4SLek=
Created: 20240403071439
Publish: 20240403071439
Activate: 20240403071439
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-62402-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-62402-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-62402-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-62402-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-64693-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 7P6uc0CngVMT1f0ny7UNqET6T+uaG6jOPwQuLEIZaw0=
Created: 20240403071439
Publish: 20240403071439
Activate: 20240403071439
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-64693-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-64693-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-64693-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-64693-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-62402-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 62402, for 10.in-addr.arpa.
; Created: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Publish: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Activate: 20240403071439 (Wed Apr  3 10:14:39 2024)
10.in-addr.arpa. IN DNSKEY 256 3 15 hz2Cqx+A0W/t9uFAZ8SH9Zaoj59QWDBlD2TXt8x720s=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-62402-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-62402-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-62402-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-62402-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-64693-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 64693, for 10.in-addr.arpa.
; Created: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Publish: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Activate: 20240403071439 (Wed Apr  3 10:14:39 2024)
10.in-addr.arpa. IN DNSKEY 257 3 15 4xGVntgXL372RUkl0lKYblo9LsjbIeya4YM7XcUcm3I=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-64693-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-64693-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-64693-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-64693-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61678-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 3w5Jyfzn8I4nvGKW0lnKNfeKDw5Jt4MLh02VG3b75D4=
Created: 20240403071439
Publish: 20240403071439
Activate: 20240403071439
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61678-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61678-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-61678-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61678-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61161-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 6IoHUlw1APiSqyOzhy5WUxDMHl7YI+3Yfi+vVELLffk=
Created: 20240403071439
Publish: 20240403071439
Activate: 20240403071439
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61161-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61161-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-61161-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61161-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61678-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 61678, for 17.10.in-addr.arpa.
; Created: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Publish: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Activate: 20240403071439 (Wed Apr  3 10:14:39 2024)
17.10.in-addr.arpa. IN DNSKEY 256 3 15 d2H6VIRxos2Rc3+Q6LxU5Zg7fYyqumFg9UcEOQrY3wc=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61678-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61678-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-61678-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61678-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61161-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 61161, for 17.10.in-addr.arpa.
; Created: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Publish: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Activate: 20240403071439 (Wed Apr  3 10:14:39 2024)
17.10.in-addr.arpa. IN DNSKEY 257 3 15 fwNKxEmsPcB0jduOrROiv41wkuS4Rw2ZNaBTuT5kS8I=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-61161-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61161-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-61161-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-61161-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-45849-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: fBaJQ84MLrlymGNTDtTzsP6iwRf+es3VbWDS4h2VXoQ=
Created: 20240403162534
Publish: 20240403162534
Activate: 20240403162534
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-45849-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-45849-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-45849-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-45849-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-48088-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: VsfsO2k+TbjKGkC988gC98/YpfJPWDRldNTGJqoVFTI=
Created: 20240403162534
Publish: 20240403162534
Activate: 20240403162534
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-48088-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-48088-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-48088-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-48088-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-45849-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 45849, for 18.10.in-addr.arpa.
; Created: 20240403162534 (Wed Apr  3 19:25:34 2024)
; Publish: 20240403162534 (Wed Apr  3 19:25:34 2024)
; Activate: 20240403162534 (Wed Apr  3 19:25:34 2024)
18.10.in-addr.arpa. IN DNSKEY 256 3 15 NkA4kTg5NWRe0UIf85xoo29h5BqHQ73t8owjBUnD3Wc=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-45849-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-45849-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-45849-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-45849-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K18-10-in-addr-arpa--015-48088-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 48088, for 18.10.in-addr.arpa.
; Created: 20240403162534 (Wed Apr  3 19:25:34 2024)
; Publish: 20240403162534 (Wed Apr  3 19:25:34 2024)
; Activate: 20240403162534 (Wed Apr  3 19:25:34 2024)
18.10.in-addr.arpa. IN DNSKEY 257 3 15 FIbgakRzqSt4KPefJ4a/fb6qTjM3qp+hAI2bVvVzCOs=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K18-10-in-addr-arpa--015-48088-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-48088-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K18-10-in-addr-arpa--015-48088-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K18-10-in-addr-arpa--015-48088-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-25221-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: qyl7LCryI2Ed9x2W7qs7JkI1v8sA+cQdj7EA+II/10Q=
Created: 20240403071439
Publish: 20240403071439
Activate: 20240403071439
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-25221-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-25221-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-25221-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-25221-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-16176-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: AKFsm/ZTmW7kF/F8FcThD1ZqiUqVrNsObtWcp7gmyzg=
Created: 20240403071439
Publish: 20240403071439
Activate: 20240403071439
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-16176-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-16176-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-16176-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-16176-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-25221-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 25221, for epl-infra.net.
; Created: 20240403071439 (Wed Apr  3 10:14:39 2024)
; Publish: 20240403071439 (Wed Apr  3 10:14:39 2024)
;

-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
2MnFa5LkE80EIykUCBu5/nh+Ow0TJ3Ov3EjfpuN6cH0=
LilBoiPeepLikesBenzTruck
...
# us-west.epl-infra.net.	IN	DS	45395 15 2 F54177A223B1B405964D8E32A611F0AAC960B24F808FAC2AB69D621E2C6F7C88
#
maybe_update_dns_file  /run/named/private-epl-infra.net.zone
# $TTL 3600
...
maybe_update_dns_file  /run/named/private-10.in-addr.arpa.zone
# $TTL 3600
...
# ns1.us-west.epl-infra.net.	IN	A	10.17.0.13
# ns2.us-west.epl-infra.net.	IN	A	10.17.0.12
# server-e.us-west.epl-infra.net.	IN	A	10.18.0.10
# server-f.us-west.epl-infra.net.	IN	A	10.18.0.11
...
maybe_update_dns_file  /run/named/private-us-west.epl-infra.net.zone
# $TTL 3600
...
# 18.10.in-addr.arpa.	IN	NS	ns2.us-west.epl-infra.net.
#
# 10.0.18.10.in-addr.arpa.	IN	PTR	server-e.us-west.epl-infra.net.
# 11.0.18.10.in-addr.arpa.	IN	PTR	server-f.us-west.epl-infra.net.
maybe_update_dns_file  /run/named/private-18.10.in-addr.arpa.zone
# $TTL 3600
# epl-infra.net.	IN	SOA	ns1.epl-infra.net. epl-infra.net. (
#  SERIAL_TO_REPLACE ; Serial
#  3600 ; Refresh
...
zpool import -af
provision_zfs_dataset rpool acme /var/lib/acme 128k on no b2RWl3wqDjtfWfk2FAMA9pD7L9FRRlGWpYpufwStOT
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool mon-am /srv/volumes/mon-am 4k on yes 7i6f7PGlGnceSJdG7utK9ZA92GUcreLEmC4Q8J2hxN
provision_zfs_dataset rpool mon-default /srv/volumes/mon-default 4k on yes kGivARlFw8Rz9h5GZDkN2WnEMXtZiLVHOT0PFjygcM
provision_zfs_dataset rpool nats1 /srv/volumes/nats1 4k on yes Li3KpBXklprhznpRTklu7wnskGl09dzrQJhSIXAT3H
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no Me62sDSOdoshlJZhat7PGVyvNtv0covaCAe0Dyj8y9
provision_zfs_dataset rpool vault /var/lib/vault 4k on no uVIepaq5Ex0Guf9b8v2Y4CTB7t6wjAfAdLwsZ0o4S6
...
wg set wg0 peer "qCP/UmaWNSCIhx4gxVpVO5y8sp2guYinsyYzzc/+R1Q=" allowed-ips "172.21.8.11/32,10.18.0.10/32"

wg set wg0 peer "sSR1NlYbYg+kOvXVyFxd3gomv6UOIXf2/Vpnk/0sV0w=" allowed-ips "172.21.8.13/32,10.18.0.11/32"
  ...
  services.frr.bgp = {
      ...
      config = ''
        ...
        router bgp 64529
          ...
          neighbor 10.17.0.12 password bVrwZ1i48XPMP13eRNNlidfDf7WyVYXQ9bc2YSEPxG
          neighbor 10.17.0.12 bfd
          neighbor 172.21.8.11 remote-as 4201179658
          neighbor 172.21.8.11 password MqHxoMwI1nzIxEHxKDnyPXdnvV0sdxbTvfWpwWfzTa
          neighbor 172.21.8.11 bfd
          neighbor 172.21.8.13 remote-as 4201179659
          neighbor 172.21.8.13 password vwTT5LUZ3KJQajRca9f4nzBRPx7g2s0kvcqlRX3yHQ
...
cp -pu /run/keys/K10-in-addr-arpa--015-62402-private /run/dnsseckeys/K10.in-addr.arpa.+015+62402.private
cp -pu /run/keys/K10-in-addr-arpa--015-64693-private /run/dnsseckeys/K10.in-addr.arpa.+015+64693.private
cp -pu /run/keys/K10-in-addr-arpa--015-62402-key /run/dnsseckeys/K10.in-addr.arpa.+015+62402.key
cp -pu /run/keys/K10-in-addr-arpa--015-64693-key /run/dnsseckeys/K10.in-addr.arpa.+015+64693.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-61678-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+61678.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-61161-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+61161.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-61678-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+61678.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-61161-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+61161.key
cp -pu /run/keys/K18-10-in-addr-arpa--015-45849-private /run/dnsseckeys/K18.10.in-addr.arpa.+015+45849.private
cp -pu /run/keys/K18-10-in-addr-arpa--015-48088-private /run/dnsseckeys/K18.10.in-addr.arpa.+015+48088.private
cp -pu /run/keys/K18-10-in-addr-arpa--015-45849-key /run/dnsseckeys/K18.10.in-addr.arpa.+015+45849.key
cp -pu /run/keys/K18-10-in-addr-arpa--015-48088-key /run/dnsseckeys/K18.10.in-addr.arpa.+015+48088.key
cp -pu /run/keys/Kepl-infra-net--015-25221-private /run/dnsseckeys/Kepl-infra.net.+015+25221.private
cp -pu /run/keys/Kepl-infra-net--015-16176-private /run/dnsseckeys/Kepl-infra.net.+015+16176.private
cp -pu /run/keys/Kepl-infra-net--015-25221-key /run/dnsseckeys/Kepl-infra.net.+015+25221.key
cp -pu /run/keys/Kepl-infra-net--015-16176-key /run/dnsseckeys/Kepl-infra.net.+015+16176.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-00187-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+00187.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-45395-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+45395.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-00187-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+00187.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-45395-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+45395.key
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"4fa96157e09c4d72e4c77c880125ca9f91e18462005646b3bacae79edac49dde\",hostname=\"server-d\"} $BOOT_TIME
" > $METRICS_FILE.tmp
