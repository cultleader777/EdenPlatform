...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
7y7oz0vD1WxxZTQrSDgOAhE71UGJTsDV37f5kCFrRmA=
LilBoiPeepLikesBenzTruck
...
while ! ${pkgs.consul}/bin/consul kv get epl-interdc-routes/dc1/10.17.1.0p24
do
  sleep 7
done

exec ${pkgs.consul}/bin/consul watch -type=key -key=epl-interdc-routes/dc1/10.17.1.0p24 /run/current-system/sw/bin/epl-process-route-data
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-f.dc1.us-west.aws-single-dc-multisub:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-f.dc1.us-west.aws-single-dc-multisub: \w\a\]\u@server-f.dc1.us-west.aws-single-dc-multisub:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-f.dc1.us-west.aws-single-dc-multisub:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"d3a6455b36b97fe7fa0b4193ed64fc9527ff421a58b51b23fad5acd49b36ecd2\",hostname=\"server-f\"} $BOOT_TIME
" > $METRICS_FILE.tmp
