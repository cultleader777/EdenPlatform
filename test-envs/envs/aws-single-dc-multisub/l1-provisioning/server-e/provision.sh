...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
XxPpNIG/uMzai0Z/WRCaeELwJqJuzxifoIHD/tx9aPo=
LilBoiPeepLikesBenzTruck
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-e.dc1.us-west.aws-single-dc-multisub:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-e.dc1.us-west.aws-single-dc-multisub: \w\a\]\u@server-e.dc1.us-west.aws-single-dc-multisub:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-e.dc1.us-west.aws-single-dc-multisub:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"cd072799aa7204ac296ce07e7a5f85fffd68d41fa778adea4fa8bae989c7f3a5\",hostname=\"server-e\"} $BOOT_TIME
" > $METRICS_FILE.tmp
