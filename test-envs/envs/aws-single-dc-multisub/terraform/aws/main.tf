...
variable "project_name" {
    type = string
    default = "aws-single-dc-multisub"
}
...
resource "aws_subnet" "router_subnet_dc1" {
    provider = aws.us-west-1

    vpc_id = aws_vpc.epl-vpc-us-west-1.id
    cidr_block = "10.17.252.0/22"
...
resource "aws_subnet" "subnet_10_17_1_0p24" {
    provider = aws.us-west-1

    vpc_id = aws_vpc.epl-vpc-us-west-1.id
    cidr_block = "10.17.1.0/24"
    availability_zone = "us-west-1b"
    ipv6_cidr_block = "${cidrsubnet(aws_vpc.epl-vpc-us-west-1.ipv6_cidr_block, 8, 18)}"
}

resource "aws_route_table_association" "igw-subnet_10_17_1_0p24" {
    provider = aws.us-west-1

    subnet_id = aws_subnet.subnet_10_17_1_0p24.id
    route_table_id = aws_route_table.internet-epl-vpc-us-west-1.id
}
...
resource "aws_instance" "server-a" {
    ...
    tags = {
        Name = "aws-single-dc-multisub - server-a"
    }

    root_block_device {
      tags = {
          Name = "aws-single-dc-multisub - server-a - root"
      }
      volume_size = 20
      volume_type = "gp2"
    ...
    depends_on = [ aws_subnet.router_subnet_dc1 ]
}

resource "aws_network_interface" "server-a-dcrouter-interface" {
    provider = aws.us-west-1

    subnet_id         = aws_subnet.router_subnet_dc1.id
    private_ips       = ["10.17.252.10"]
    security_groups   = [aws_security_group.private-node-epl-vpc-us-west-1.id]
    source_dest_check = false
...
resource "aws_instance" "server-b" {
    ...
    tags = {
        Name = "aws-single-dc-multisub - server-b"
    }

    root_block_device {
      tags = {
          Name = "aws-single-dc-multisub - server-b - root"
      }
      volume_size = 20
      volume_type = "gp2"
...
resource "aws_network_interface" "server-b-dcrouter-interface" {
    provider = aws.us-west-1

    subnet_id         = aws_subnet.router_subnet_dc1.id
    private_ips       = ["10.17.252.11"]
    security_groups   = [aws_security_group.private-node-epl-vpc-us-west-1.id]
    source_dest_check = false
...
resource "aws_instance" "server-c" {
    ...
    tags = {
        Name = "aws-single-dc-multisub - server-c"
    }

    root_block_device {
      tags = {
          Name = "aws-single-dc-multisub - server-c - root"
      }
      volume_size = 20
      volume_type = "gp2"
...
resource "aws_network_interface" "server-c-dcrouter-interface" {
    provider = aws.us-west-1

    subnet_id         = aws_subnet.router_subnet_dc1.id
    private_ips       = ["10.17.252.12"]
    security_groups   = [aws_security_group.private-node-epl-vpc-us-west-1.id]
    source_dest_check = false
...
resource "aws_instance" "server-d" {
    ...
    tags = {
        Name = "aws-single-dc-multisub - server-d"
    }

    root_block_device {
      tags = {
          Name = "aws-single-dc-multisub - server-d - root"
      }
      volume_size = 20
      volume_type = "gp2"
...
resource "aws_network_interface" "server-d-dcrouter-interface" {
    provider = aws.us-west-1

    subnet_id         = aws_subnet.router_subnet_dc1.id
    private_ips       = ["10.17.252.13"]
    security_groups   = [aws_security_group.private-node-epl-vpc-us-west-1.id]
    source_dest_check = false

    attachment {
      instance     = aws_instance.server-d.id
      device_index = 1
    }
}
...
resource "aws_instance" "server-e" {
    ...
    tags = {
        Name = "aws-single-dc-multisub - server-e"
    }

    root_block_device {
      tags = {
          Name = "aws-single-dc-multisub - server-e - root"
      }
      volume_size = 20
      volume_type = "gp2"
...
resource "aws_instance" "server-f" {
    ...
    tags = {
        Name = "aws-single-dc-multisub - server-f"
    }

    root_block_device {
      tags = {
          Name = "aws-single-dc-multisub - server-f - root"
      }
      volume_size = 20
      volume_type = "gp2"
