...
EOF
mc admin policy add thisminio rw-loki2 /secrets/policy.json
...
EOF
mc admin policy add thisminio ro-loki2 /secrets/policy.json
...
EOF
mc admin policy add thisminio rw-app1 /secrets/policy.json
...
mc admin user add thisminio tempo_us_west {{ with secret "epl/data/minio/global" }}{{ .Data.data.minio_user_tempo_us_west_password }}{{ end }}
...
EOL
  ...
  group "epl-minio-global-4" {
    ...
    constraint {
      attribute = "${meta.lock_epl-minio-server-d-global}"
      operator  = ">"
      value     = "0"
    }
    network {
