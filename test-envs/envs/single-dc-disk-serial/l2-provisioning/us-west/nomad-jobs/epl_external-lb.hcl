job "external-lb" {
  ...
  group "external-lb-x86_64" {
    ...
    constraint {
      attribute = "${meta.lock_epl-ingress-us-west}"
      operator  = ">"
      value     = "0"
    }
    network {
    ...
    task "external-lb" {
      ...
      config {
        image = "openresty/openresty@sha256:f3fa9a80b4e8fae569bf85bf40ae9bb149e9f9ec1fc786aa4d34a117eedc0845"
        network_mode = "host"
        entrypoint = [
          "/local/start",
        ]
      ...
      template {
        destination = "secrets/admin-index.html.src"
        perms = "0644"
        change_mode = "script"
        change_script {
          command = "/local/onchange-index-html"
        }
        data = "{{ key \"epl-kv/admin-site-html\" }}"
      }

      template {
        destination = "secrets/nginx.conf.src"
        perms = "0644"
        change_mode = "script"
        change_script {
          command = "/local/onchange-conf"
        }
        data = "{{ key \"epl-kv/external-elb-conf\" }}"
      }

      template {
        destination = "secrets/prom-server.conf"
        perms = "644"
        data = <<EOL
...
EOL
      ...
      template {
        ...
        perms = "644"
        data = <<EOL
admin:$2a$12$cY4qA2Qchv5qnXrqkRT7KuTIR3IMmAbcea0pULQAGNImFcltlw3rS
EOL
      }

      template {
        destination = "local/onchange-conf"
        perms = "755"
        data = <<EOL
#!/bin/sh
...
EOL
      ...
      template {
        destination = "local/onchange-index-html"
        perms = "755"
        data = <<EOL
#!/bin/sh
...
EOL
      ...
      template {
        destination = "local/prometheus.lua"
        perms = "644"
        data = <<EOL
-- https://github.com/knyar/nginx-lua-prometheus/commit/10932bb06bbcc647ebe50b382d85bd6a9939804d version
--- @module Prometheus
--
...
EOL
      ...
      template {
        destination = "local/prometheus_keys.lua"
        perms = "644"
        data = <<EOL
...
EOL
      ...
      template {
        destination = "local/prometheus_resty_counter.lua"
        perms = "644"
        data = <<EOL
...
/local/onchange-conf noreload
/local/onchange-index-html

exec /usr/bin/openresty -g 'daemon off;' -c /secrets/nginx.conf
EOL
      }
    }
  }
