...
SEC_4=$( vault kv get -format=json epl/tempo/us-west | jq -Srj '.data.data.minio_bucket_password' | sed 's/$/\\n/g' | tr -d '\n' | sed 's/..$//' )
MERGED_SECRET=$( echo "{\"admin_password\":\"$SEC_1\",\"minio_user_docker_registry_password\":\"$SEC_2\",\"minio_user_loki_main_password\":\"$SEC_3\",\"minio_user_tempo_us_west_password\":\"$SEC_4\"} $CURRENT_SECRET" | jq -S -s add )
if [ "$MERGED_SECRET" != "$CURRENT_SECRET" ];
then
  echo "Secret $SEC_KEY changed"
  echo "$MERGED_SECRET" | vault kv put $SEC_KEY -
