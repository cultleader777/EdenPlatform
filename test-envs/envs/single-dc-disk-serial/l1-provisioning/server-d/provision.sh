...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
{
  "acl": {
    ...
    "enabled": true,
    "tokens": {
      "agent": "addf6aba-8c6e-420d-af5e-cf9a0b4bb7d5",
      "default": "4ac7c3bb-2ca8-4df0-bb76-ec9fb4f6cc28"
    }
  },
  "addresses": {
    "dns": "127.0.0.1",
  ...
  "encrypt": "+2bQ7Jf/T+fDgmYQtuivYbcwXXR4lMPDgY661jrjduQ=",
...
consul {
    ...
    cert_file = ""
    key_file = ""
    token = "63c6dfc6-8e3e-4def-ae2e-71b45feca764"
...
vault {
    ...
    namespace = ""
    token = "hvs.CAESIBRcQiy82cWABz_uhSe-4RVdzrndxfgNhYcGtkudejKqGh4KHGh2cy51SmhhS1cyVGdUZUlBbG9HNVV0N0puYk4"
...
client {
  ...
  host_volume "minio-docker" {
    path = "/srv/xfs-jbods/virtio-server-d-vdb/minio-docker"
    read_only = false
  }
...
server {
    ...
    encrypt = "1nCnkYU19uB0aB7XpQZN6NumMBZtqLbSlyZ3LnqO+fc="
...
service_registration "consul" {
  ...
  token = "2492c978-5889-4f06-9273-49c1b82a9807"
}
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
x+hEkOoegb3IS2xGovUVb7B3sUeLqCu2VtJMBQPZSa0=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
eoAL8QAXqEu/l/+MxURtIpG448lRCGejdpM9c3k40CY=
LilBoiPeepLikesBenzTruck
...
MHcCAQEEICV4Y/LjkK6z4fbaR9UXeuJPW2aTNeNVzYyXiQL1rm4LoAoGCCqGSM49
AwEHoUQDQgAEh9EQv5LL1vZS7jP33e3JhQutunW1yS5RtuJpeKslHm1EdUNjNYPh
MewC9kfztrtiS7HQfY35vwxQIzxaSil1PQ==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
# NIX REGION secret_value_K10-in-addr-arpa--015-15948-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 0cxgLeY5704kvtz+qb561DfrazCWYYKWsnKpaG4Wi6w=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-15948-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-15948-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-15948-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-15948-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-53719-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: BZJL24mUe85b/+wW0f43nuizyj2Q5DDo/QYA34QGSeo=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-53719-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-53719-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-53719-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-53719-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-15948-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 15948, for 10.in-addr.arpa.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
10.in-addr.arpa. IN DNSKEY 256 3 15 ZhnXAx4yUVfsGoiv+suTTemG8j85Uie2+hU9x2+npl4=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-15948-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-15948-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-15948-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-15948-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K10-in-addr-arpa--015-53719-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 53719, for 10.in-addr.arpa.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
10.in-addr.arpa. IN DNSKEY 257 3 15 YBxGb3PUdnudpitkWV1lZvvWhdS0fb5brdGb7TGZRUA=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K10-in-addr-arpa--015-53719-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-53719-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K10-in-addr-arpa--015-53719-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K10-in-addr-arpa--015-53719-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-58119-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: nVZrXRByP/BVnkEgS5G2J8mEKQ9LaJfdQSbaaE80vas=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-58119-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-58119-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-58119-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-58119-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-14489-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: Wnw/S1K3/Jh1+t7XnH3sc29I0HfoWENyKUT/pGP9SiQ=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-14489-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-14489-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-14489-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-14489-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-58119-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 58119, for 17.10.in-addr.arpa.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
17.10.in-addr.arpa. IN DNSKEY 256 3 15 qMtyrh5XQOqntALYWJ4LeTKwgjl9F1mpu0ZvF9xawzU=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-58119-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-58119-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-58119-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-58119-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_K17-10-in-addr-arpa--015-14489-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 14489, for 17.10.in-addr.arpa.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
17.10.in-addr.arpa. IN DNSKEY 257 3 15 eGFDMgkxWDsWIhdC6UzVBcWa21BEn0uG3EOTrewnnqc=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_K17-10-in-addr-arpa--015-14489-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-14489-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/K17-10-in-addr-arpa--015-14489-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/K17-10-in-addr-arpa--015-14489-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-31451-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: oGKg4rcITsguz+xsb+OImqTuqJlT3kVM788PbdOeVF0=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-31451-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-31451-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-31451-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-31451-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-02776-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: OyrYw7yLwkM6KKfAvolMqHw6V8MP6Iag06/5+OEhae4=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-02776-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-02776-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-02776-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-02776-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-31451-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 31451, for epl-infra.net.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
epl-infra.net. IN DNSKEY 256 3 15 xhGnkY8Itknz/3ktJgEbpCWu10jPsb8eOaiWh0iHcIU=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-31451-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-31451-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-31451-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-31451-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kepl-infra-net--015-02776-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 2776, for epl-infra.net.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
epl-infra.net. IN DNSKEY 257 3 15 fLtoDrcSUwvJd8s/HI5mWWHmz1aS9j668RC5EdpDeOw=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kepl-infra-net--015-02776-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-02776-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kepl-infra-net--015-02776-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kepl-infra-net--015-02776-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-04422-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: nuCnG+aI+qhe5VhxJjI/Kt0t+VHc3GLmMiLGX4FF3bY=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-04422-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-04422-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-04422-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-04422-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-21237-private START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
Private-key-format: v1.3
Algorithm: 15 (ED25519)
PrivateKey: 0VfuJbbWBcW6lmwlaH2wzFReEniZ6HSw4NVSP+NE9Tc=
Created: 20240615133018
Publish: 20240615133018
Activate: 20240615133018
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-21237-private END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-21237-private || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-21237-private')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-21237-private
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-04422-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a zone-signing key, keyid 4422, for us-west.epl-infra.net.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
us-west.epl-infra.net. IN DNSKEY 256 3 15 7+llSC5J4y7MuSL2QM/2OC3uOp9dOMkMfwBXTyID+K4=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-04422-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-04422-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-04422-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-04422-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM

# NIX REGION secret_value_Kus-west-epl-infra-net--015-21237-key START
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
; This is a key-signing key, keyid 21237, for us-west.epl-infra.net.
; Created: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Publish: 20240615133018 (Sat Jun 15 16:30:18 2024)
; Activate: 20240615133018 (Sat Jun 15 16:30:18 2024)
us-west.epl-infra.net. IN DNSKEY 257 3 15 wneIDfnzKyR4QhXc3ZfVQ5muabKdyrdxLJ/O2FQW9iE=
LilBoiPeepLikesBenzTruck

# NIX REGION secret_value_Kus-west-epl-infra-net--015-21237-key END
if id -u named &>/dev/null && id -g named &>/dev/null; then
  chown named $TMP_SECRET_PATH
  chgrp named $TMP_SECRET_PATH
  unset NEEDS_MOVE
  cmp --silent $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-21237-key || NEEDS_MOVE=true
  [ "$(stat -c '%A:%U:%G' $TMP_SECRET_PATH)" == "$(stat -c '%A:%U:%G' '/run/keys/Kus-west-epl-infra-net--015-21237-key')" ] || NEEDS_MOVE=true
  [ -n "$NEEDS_MOVE" ] && mv -f $TMP_SECRET_PATH /run/keys/Kus-west-epl-infra-net--015-21237-key
fi
rm -f $TMP_SECRET_PATH || true
TMP_SECRET_PATH=/run/tmpsec-$RANDOM
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

...

ICfR5h0OZxnm1VrEB+yav1qZV4AoK5QhNlrbTqrP7G4O
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
addf6aba-8c6e-420d-af5e-cf9a0b4bb7d5
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

Llvzp5UdBLhNku6Ti4LklEGCVPJ4zNuO
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEINZvJ3vi4CVRQSgOVxlpFWeJtnfJvTzS8WBAHp3c4ImKoAoGCCqGSM49
AwEHoUQDQgAExo5lPEdXSmWwI3iYmpnAQvOoI5faBX6CaEvRhj+CMMoLVM2++TZR
/v0m3I0VDVu/NkvNyTmvQhH4zz7olSzN8Q==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIGkq/csm/x9Y6R9wwYApRveRf733ZqexJlZoH+O+h29qoAoGCCqGSM49
AwEHoUQDQgAECBySr8IJhU4Dm0bbfO1Wwqq+Ms+8TqpqP6Rc3S1KGDLEWg0ifulU
/VQER10TbhEeKNcALDpHiIOgbDnpVYYGDA==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

...
gglsb2NhbGhvc3SHBH8AAAEwCgYIKoZIzj0EAwIDSAAwRQIgVY1JSFYZ0XgoB4WB
YX8bhfbG2jV+uWbHVUmRGE9rFpYCIQDoJiLM3DYDNJRqgGJVxAsNGZ6fatrMAZUc
GxTk0ObeFQ==
-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIMYOy8EwhLXoXJaxrtfYEoJn40jV+D4JEr3rIHLpStcSoAoGCCqGSM49
AwEHoUQDQgAEGBcCrf/wGZJUk3KO+9DRmPfqMCzVoy7l8tZPNTN2VI34R7sXVV0m
JZ0dY8IUpYxHZy5eA5KlLAfSZu8WAmn6/A==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
qHGiO2/C97E8sckRyPxfVdYffqw728ibLkmVdod6LkM=
LilBoiPeepLikesBenzTruck
...
# us-west.epl-infra.net.	IN	DS	21237 15 2 BD58C4A8E3E1366776D80F08389C0FAABD8F6F412558C38CCA2BBEB3ECEAAED2
#
maybe_update_dns_file  /run/named/private-epl-infra.net.zone
# $TTL 3600
...
zpool import -af
provision_zfs_dataset rpool acme /var/lib/acme 128k on no BpozA2pm5mn9Fjx6t2l9YIFzC6NTOgnu44d1hGY1oM
provision_zfs_dataset rpool ch-keeper /srv/volumes/ch-keeper 4k on yes FprzR4R4IMF4hYKOD2EiZmTa0rsVupOsPhsTbGJ3q5
provision_zfs_dataset rpool clickhouse /srv/volumes/clickhouse 4k on yes Q0vXL66F37JyjLSRlx64Xl5bKceX8LT2KrN5jSVS6n
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool mon-am /srv/volumes/mon-am 4k on yes SuvwyYIsKTh9kpRHa5wh2sgCAFUSrWYlmkXkM5Ff8t
provision_zfs_dataset rpool moonbeam-dev /srv/volumes/moonbeam-dev 4k on yes qMlJwWSsNRllcAnfcjxVZ0PE2D6TT8AXUmQ3Z6n0g0
provision_zfs_dataset rpool nats1 /srv/volumes/nats1 4k on yes 9dJtLmkTJMB60AgKhCh7oKDjZ9ndXwhF26drRQC9Qc
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no 4GkaJ6hjkxLtnkooOzQmdfaHetC216xXCTABybqzZN
provision_zfs_dataset rpool vault /var/lib/vault 4k on no WfvpCaNCY2eL0Pw0FAeeZCdcA68fUBZCLH5oT2MwZo


mkdir -p /srv/xfs-jbods/virtio-server-d-vdb
chmod 700 /srv/xfs-jbods/virtio-server-d-vdb

# we don't bother with nixos config or fstab
# because we must run l1 provisioning post boot anyway
CURR_FS_TYPE=$( lsblk -n -o FSTYPE /dev/disk/by-id/virtio-server-d-vdb )
if [ -z "$CURR_FS_TYPE" ]
then
    # if disk is empty provision xfs filesystem
    nix-shell -p xfsprogs --command 'mkfs.xfs /dev/disk/by-id/virtio-server-d-vdb'
fi

if ! mount | grep '/dev/disk/by-id/virtio-server-d-vdb'
then
    mount -o noatime /dev/disk/by-id/virtio-server-d-vdb /srv/xfs-jbods/virtio-server-d-vdb
fi

CURR_FS_SIZE=$( df --output=size -B1 /dev/disk/by-id/virtio-server-d-vdb | tail -n-1 )
CURR_DEVICE_SIZE=$( lsblk -n -o SIZE -b /dev/disk/by-id/virtio-server-d-vdb )
AT_LEAST_SIZE=$(( CURR_FS_SIZE + 536870912 ))
if [ "$AT_LEAST_SIZE" -le "$CURR_DEVICE_SIZE" ]
then
    echo "Block device /dev/disk/by-id/virtio-server-d-vdb expanded, expanding xfs filesystem..."
    nix-shell -p xfsprogs --command 'xfs_growfs /dev/disk/by-id/virtio-server-d-vdb'
fi

mkdir -p /srv/xfs-jbods/virtio-server-d-vdb/minio-docker
chmod 777 /srv/xfs-jbods/virtio-server-d-vdb/minio-docker
...
# NIX REGION firewall END
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-d.dc1.us-west.single-dc-disk-serial:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-d.dc1.us-west.single-dc-disk-serial: \w\a\]\u@server-d.dc1.us-west.single-dc-disk-serial:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-d.dc1.us-west.single-dc-disk-serial:\u:\w\007\]$PS1"
       fi
     fi
   '';
  ...
  fileSystems."/boot" =
    ...
    services.bind =
    {
        ...
        extraConfig = ''
          trust-anchors {
  epl-infra.net. initial-key 257 3 15 "fLtoDrcSUwvJd8s/HI5mWWHmz1aS9j668RC5EdpDeOw=";
  us-west.epl-infra.net. initial-key 257 3 15 "wneIDfnzKyR4QhXc3ZfVQ5muabKdyrdxLJ/O2FQW9iE=";
  10.in-addr.arpa. initial-key 257 3 15 "YBxGb3PUdnudpitkWV1lZvvWhdS0fb5brdGb7TGZRUA=";
  17.10.in-addr.arpa. initial-key 257 3 15 "eGFDMgkxWDsWIhdC6UzVBcWa21BEn0uG3EOTrewnnqc=";
  in-addr.arpa. initial-key 257 3 15 "FtdGrccSFtATCprxlY3EFRx3ryE0g9e0kDtB8CZEnNw=";
};
...
# peers
wg set wg0 peer "6eta+uZZDCSBaDirTjxFCcVH+fBi8MoVBEhVUL1VgWI=" allowed-ips "172.21.7.254/32"
...
cp -pu /run/keys/K10-in-addr-arpa--015-15948-key /run/dnsseckeys/K10.in-addr.arpa.+015+15948.key
cp -pu /run/keys/K10-in-addr-arpa--015-53719-key /run/dnsseckeys/K10.in-addr.arpa.+015+53719.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-58119-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+58119.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-14489-private /run/dnsseckeys/K17.10.in-addr.arpa.+015+14489.private
cp -pu /run/keys/K17-10-in-addr-arpa--015-58119-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+58119.key
cp -pu /run/keys/K17-10-in-addr-arpa--015-14489-key /run/dnsseckeys/K17.10.in-addr.arpa.+015+14489.key
cp -pu /run/keys/Kepl-infra-net--015-31451-private /run/dnsseckeys/Kepl-infra.net.+015+31451.private
cp -pu /run/keys/Kepl-infra-net--015-02776-private /run/dnsseckeys/Kepl-infra.net.+015+02776.private
cp -pu /run/keys/Kepl-infra-net--015-31451-key /run/dnsseckeys/Kepl-infra.net.+015+31451.key
cp -pu /run/keys/Kepl-infra-net--015-02776-key /run/dnsseckeys/Kepl-infra.net.+015+02776.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-04422-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+04422.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-21237-private /run/dnsseckeys/Kus-west.epl-infra.net.+015+21237.private
cp -pu /run/keys/Kus-west-epl-infra-net--015-04422-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+04422.key
cp -pu /run/keys/Kus-west-epl-infra-net--015-21237-key /run/dnsseckeys/Kus-west.epl-infra.net.+015+21237.key
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"1276091f89dd75bc5f771f5a1784ec2dd879e085cdb79acba641e7501daaac6c\",hostname=\"server-d\"} $BOOT_TIME
" > $METRICS_FILE.tmp
