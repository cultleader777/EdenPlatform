...
client {
  ...
  host_volume "minio-docker" {
    path = "/srv/xfs-jbods/virtio-server-a-vdb/minio-docker"
    read_only = false
  }
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
bPQnoDaW1jcLcYmKMMtMmovY84fzVuzMMbnyYqh6yg0=
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
LilBoiPeepLikesBenzTruck
...
cat > $TMP_SECRET_PATH <<'LilBoiPeepLikesBenzTruck'
-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIFpIGVJDYlJKI1Jzj85bBNeDohjH+gXnUGGaGABRQTlGoAoGCCqGSM49
AwEHoUQDQgAEq4dwRGi/o3v30BLXP+OjWngsgg5NYUEf+Rvy5J3JkQqnyWI3xstu
ioMvAClS2xEHN0ZBPQgAfSunInHEIKxKRw==
-----END EC PRIVATE KEY-----
LilBoiPeepLikesBenzTruck
...
zpool import -af
zpool list -H -o name | grep 'extra' || zpool create extra -m none -o autoexpand=on -o ashift=12 mirror /dev/disk/by-id/virtio-server-a-vdc /dev/disk/by-id/virtio-server-a-vdd mirror /dev/disk/by-id/virtio-server-a-vde /dev/disk/by-id/virtio-server-a-vdf spare /dev/disk/by-id/virtio-server-a-vdg cache /dev/disk/by-id/virtio-server-a-vdh log mirror /dev/disk/by-id/virtio-server-a-vdi /dev/disk/by-id/virtio-server-a-vdj
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '/^\s+logs/q' | sed -E '/^\s+cache/q' | sed -E '/^\s+spares/q' | grep -E '^\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "4" ];
then
    zpool add extra mirror /dev/disk/by-id/virtio-server-a-vde /dev/disk/by-id/virtio-server-a-vdf
fi
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '1,/^\s+logs/ d' | sed -E '/^\s+cache/q' | sed -E '/^\s+spares/q' | grep -E '\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "1" ];
then
    zpool add extra log /dev/disk/by-id/virtio-server-a-vdi
fi
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '1,/^\s+logs/ d' | sed -E '/^\s+cache/q' | sed -E '/^\s+spares/q' | grep -E '\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "2" ];
then
    zpool add extra log /dev/disk/by-id/virtio-server-a-vdj
fi
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '1,/^\s+cache/ d' | sed -E '/^\s+spares/q' | grep -E '\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "1" ];
then
    zpool add extra cache /dev/disk/by-id/virtio-server-a-vdh
fi
ZPOOL_DISK_COUNT=$( zpool status -P extra | awk '/^config:/{f=1;next} /^errors:/{f=0} f' | sed -E '1,/^\s+spares/ d' | grep -E '^\s+/dev/' | wc -l )
if [ "$ZPOOL_DISK_COUNT" -lt "1" ];
then
    zpool add extra spare /dev/disk/by-id/virtio-server-a-vdg
fi
zpool online -e extra /dev/disk/by-id/virtio-server-a-vdc
zpool online -e extra /dev/disk/by-id/virtio-server-a-vdd
zpool online -e extra /dev/disk/by-id/virtio-server-a-vde
zpool online -e extra /dev/disk/by-id/virtio-server-a-vdf
zpool online -e extra /dev/disk/by-id/virtio-server-a-vdi
zpool online -e extra /dev/disk/by-id/virtio-server-a-vdj
provision_zfs_dataset extra bigone /srv/volumes/bigone 4k on yes OFfyc9SbxxFErH25gV1bk2FaBm4FRorvh3sJH1n52f
provision_zfs_dataset rpool ch-keeper /srv/volumes/ch-keeper 4k on yes n2quALYF2dElDayJUr2ixP9L6lltQKtAYU0SN1pwMO
provision_zfs_dataset rpool consul /var/lib/consul 4k on no SwlHXB0Z2e3cnqEI4Rhd1jldMQX6rRURBfiyr0Y8GY
provision_zfs_dataset rpool docker /var/lib/docker 128k on no
provision_zfs_dataset rpool mon-am /srv/volumes/mon-am 4k on yes F7KhnZ8xTr4oKQdEkaMAamvtVpcMcBRAqWri4xMnZA
provision_zfs_dataset rpool mon-default /srv/volumes/mon-default 4k on yes ASZg7pK7pZFH761xbcBE2PTsNjj8rE9FgmBuimP02s
provision_zfs_dataset rpool nomad /var/lib/nomad 4k on no 8u4T6HzzC8K4flv8cG5LzYOLuyBqY4xS3zYs8WpxMz
provision_zfs_dataset rpool pgtest1 /srv/volumes/pgtest1 4k on yes bAWf2DoJDzHszKvezmJ90EDmhL5Tq1IwEVSppSri47


mkdir -p /srv/xfs-jbods/virtio-server-a-vdb
chmod 700 /srv/xfs-jbods/virtio-server-a-vdb

# we don't bother with nixos config or fstab
# because we must run l1 provisioning post boot anyway
CURR_FS_TYPE=$( lsblk -n -o FSTYPE /dev/disk/by-id/virtio-server-a-vdb )
if [ -z "$CURR_FS_TYPE" ]
then
    # if disk is empty provision xfs filesystem
    nix-shell -p xfsprogs --command 'mkfs.xfs /dev/disk/by-id/virtio-server-a-vdb'
fi

if ! mount | grep '/dev/disk/by-id/virtio-server-a-vdb'
then
    mount -o noatime /dev/disk/by-id/virtio-server-a-vdb /srv/xfs-jbods/virtio-server-a-vdb
fi

CURR_FS_SIZE=$( df --output=size -B1 /dev/disk/by-id/virtio-server-a-vdb | tail -n-1 )
CURR_DEVICE_SIZE=$( lsblk -n -o SIZE -b /dev/disk/by-id/virtio-server-a-vdb )
AT_LEAST_SIZE=$(( CURR_FS_SIZE + 536870912 ))
if [ "$AT_LEAST_SIZE" -le "$CURR_DEVICE_SIZE" ]
then
    echo "Block device /dev/disk/by-id/virtio-server-a-vdb expanded, expanding xfs filesystem..."
    nix-shell -p xfsprogs --command 'xfs_growfs /dev/disk/by-id/virtio-server-a-vdb'
fi

mkdir -p /srv/xfs-jbods/virtio-server-a-vdb/minio-docker
chmod 777 /srv/xfs-jbods/virtio-server-a-vdb/minio-docker
   ...
   programs.bash.promptInit = ''
     ...
     if [ "$TERM" != "dumb" ] || [ -n "$INSIDE_EMACS" ]; then
       ...
       if [ -n "$INSIDE_EMACS" ]; then
         ...
         PS1="\n\[\033[$PROMPT_COLOR\][\u@server-a.dc1.us-west.single-dc-disk-serial:\w]\\$\[\033[0m\] "
       else
         PS1="\n\[\033[$PROMPT_COLOR\][\[\e]0;\u@server-a.dc1.us-west.single-dc-disk-serial: \w\a\]\u@server-a.dc1.us-west.single-dc-disk-serial:\w]\\$\[\033[0m\] "
       fi
       if test "$TERM" = "xterm"; then
         PS1="\[\033]2;server-a.dc1.us-west.single-dc-disk-serial:\u:\w\007\]$PS1"
       fi
     fi
   '';
...
then
  ...
  echo "
epl_l1_provisioning_last_hash{hash=\"5c627ded4757764b3c162c3a4bdbb3e8bc22643c4ccf23ec2a70c4b4445d7fb4\",hostname=\"server-a\"} $BOOT_TIME
" > $METRICS_FILE.tmp
  ...
  echo '
epl_l1_provisioning_expected_hash{hash="5c627ded4757764b3c162c3a4bdbb3e8bc22643c4ccf23ec2a70c4b4445d7fb4",hostname="server-a"} 1
epl_l1_provisioning_expected_hash{hash="50e367b99453a2a73e58a36d21886758d819044dd894732d9ad02db0a16b8d29",hostname="server-b"} 1
epl_l1_provisioning_expected_hash{hash="8b41ca50232e3ecf24c40542b2e1b0fe7d422240a095d85cf7bc68c6125bf71b",hostname="server-c"} 1
epl_l1_provisioning_expected_hash{hash="209ff7d5ed9ff9679b19eea89088a343466507e0504018a184b9c3b87445e12a",hostname="server-d"} 1
