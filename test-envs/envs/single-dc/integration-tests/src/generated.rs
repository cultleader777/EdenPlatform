...
#[tokio::test]
async fn bb_depl_moonbeam_dev_service_epl_bb_moonbeam_prom_instances_exist() -> Result<(), Box<dyn Error>> {
    let ips_and_dns = [
        ("epl-bb-moonbeam-prom.service.consul", 1),
...
#[tokio::test]
async fn clickhouse_keeper_testkeeper_prometheus_metrics_gathered() -> Result<(), Box<dyn Error>> {
    assert!(does_prometheus_metric_exist("10.17.0.10", 9090, "ClickHouseAsyncMetrics_KeeperZnodeCount{job=\"epl-ch-keeper-testkeeper\"}").await?);

    Ok(())
}

#[tokio::test]
async fn clickhouse_keeper_testkeeper_tcp_socket_open() -> Result<(), Box<dyn Error>> {
    let sockets = [
        "10.17.0.10:9181",
        "10.17.0.11:9181",
        "10.17.0.13:9181",
...
#[tokio::test]
async fn clickhouse_server_testch_native_port_open() -> Result<(), Box<dyn Error>> {
    let sockets = [
        "10.17.0.11:8120",
        "10.17.0.12:8120",
        "10.17.0.13:8120",
...
#[tokio::test]
async fn clickhouse_server_testch_prometheus_metrics_gathered() -> Result<(), Box<dyn Error>> {
    assert!(does_prometheus_metric_exist("10.17.0.10", 9090, "ClickHouseProfileEvents_FileOpen{job=\"epl-clickhouse-testch\"}").await?);
...
async fn dns_public_ingress_records_have_rev_in_master() -> Result<(), Box<dyn Error>> {
    let ptr_and_dns: &[(&str, &[&str])] = &[
        ("12.77.77.77.in-addr.arpa.", &["www.epl-infra.net."]),
        ("13.77.77.77.in-addr.arpa.", &["www.epl-infra.net."]),
...
async fn dns_public_ingress_records_resolve_from_master() -> Result<(), Box<dyn Error>> {
    let ips_and_dns: &[(&str, &[&str])] = &[
        ("www.epl-infra.net", &["77.77.77.12", "77.77.77.13"]),
...
async fn nats_cluster_main_nats_prometheus_metrics_gathered() -> Result<(), Box<dyn Error>> {
    assert!(does_prometheus_metric_exist("10.17.0.10", 9090, "gnatsd_varz_jetstream_meta_cluster_size{job=\"epl-nats-main-nats-prom\"}").await?);
...
#[tokio::test]
async fn nomad_job_region_us_west_job_app_frontend_other_logs_in_main() -> Result<(), Box<dyn Error>> {
    assert!(check_if_loki_stream_exists("10.17.0.10:53", "epl-loki-main-loki-reader.service.consul", 3012, "{source_type=\"nomad_docker\",job_name=\"app-frontend-other\"}").await?);

    Ok(())
}

#[tokio::test]
async fn nomad_job_region_us_west_job_app_frontend_test_logs_in_main() -> Result<(), Box<dyn Error>> {
    assert!(check_if_loki_stream_exists("10.17.0.10:53", "epl-loki-main-loki-reader.service.consul", 3012, "{source_type=\"nomad_docker\",job_name=\"app-frontend-test\"}").await?);

    Ok(())
}

#[tokio::test]
async fn nomad_job_region_us_west_job_app_test_hello_world_logs_in_main() -> Result<(), Box<dyn Error>> {
    assert!(check_if_loki_stream_exists("10.17.0.10:53", "epl-loki-main-loki-reader.service.consul", 3012, "{source_type=\"nomad_docker\",job_name=\"app-test-hello-world\"}").await?);

    Ok(())
}

#[tokio::test]
async fn nomad_job_region_us_west_job_bb_moonbeam_dev_logs_in_main() -> Result<(), Box<dyn Error>> {
    assert!(check_if_loki_stream_exists("10.17.0.10:53", "epl-loki-main-loki-reader.service.consul", 3012, "{source_type=\"nomad_docker\",job_name=\"bb-moonbeam-dev\"}").await?);

    Ok(())
}

#[tokio::test]
async fn nomad_job_region_us_west_job_ch_keeper_testkeeper_logs_in_main() -> Result<(), Box<dyn Error>> {
    assert!(check_if_loki_stream_exists("10.17.0.10:53", "epl-loki-main-loki-reader.service.consul", 3012, "{source_type=\"nomad_docker\",job_name=\"ch-keeper-testkeeper\"}").await?);

    Ok(())
}

#[tokio::test]
async fn nomad_job_region_us_west_job_clickhouse_testch_logs_in_main() -> Result<(), Box<dyn Error>> {
    assert!(check_if_loki_stream_exists("10.17.0.10:53", "epl-loki-main-loki-reader.service.consul", 3012, "{source_type=\"nomad_docker\",job_name=\"clickhouse-testch\"}").await?);
...
#[tokio::test]
async fn pg_deployment_testdb_db_exists_testdb_a() -> Result<(), Box<dyn Error>> {
    assert!(does_prometheus_metric_exist("10.17.0.10", 9090, "pg_locks_count{job='epl-pg-testdb-pg-exp',datname='testdb_a'}").await?);
